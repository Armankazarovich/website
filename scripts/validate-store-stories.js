...
const checks = [
  ...
  },
  {
    ...
    label: "manager reorders stories in one transaction without touching other fields",
    patterns: ["requireManager", "requireWriteConfirmation", "$transaction", "sortOrder: (index + 1) * 10"],
  },
  {
  ...
  },
  {
    ...
    label: "admin media client uploads, monitors and retries durable story jobs",
    patterns: [
      "ADMIN_UPLOAD_CHUNK_SIZE",
  ...
  },
  {
    ...
    label: "admin editor exposes media preparation, recovery, library, sharing and multi-related templates",
    patterns: [
      "normalizeRelations",
      "uploadStoryMediaFile",
      "retryStoryMediaUpload",
      "prepareStoryVideo",
      "rollbackStoryVideo",
      // Человеческие слова вместо техшума — замечание Армана 10.09.2026.
      "Облегчить видео",
      "Ваш файл сохранится",
      "Вернуть как было",
      "MediaWeightBadge",
