...
async function exchangeCode(req: Request, code: string) {
  ...
  if (!response.ok || payload.error || !payload.access_token) {
    throw new Error(payload.error_description || payload.error || "Яндекс не вернул OAuth token");
  }
...
export async function GET(req: Request) {
  ...
  const expectedState = req.headers.get("cookie")?.match(/(?:^|;\s*)aray_direct_oauth_state=([^;]+)/)?.[1];
  ...
  try {
    ...
    if (!state || !expectedState || decodeURIComponent(expectedState) !== state) {
      throw new Error("OAuth state не совпал, подключение Direct остановлено");
    }
    ...
    await saveSetting("yandex_direct_oauth_token", encryptSettingValue(token.access_token || ""), tenantId);
    if (token.refresh_token) await saveSetting("yandex_direct_refresh_token", encryptSettingValue(token.refresh_token), tenantId);
