"use client";
/** * /admin/products/import-prices * * Импорт прайса от поставщика (Пилорус и т.д.). * CSV формат: секции с заголовком-категорией + подзаголовок колонок + строки данных. * * Flow: * 1. Вставить CSV → "Посмотреть изменения" → POST { csv, apply: false } * 2. Просмотреть diff: matched / unmatched / changed * 3. "Применить" → POST { csv, apply: true } → обновление БД */ import { useState } from "react";
import {
  Upload,
  FileCheck,
  AlertTriangle,
...
export default function ImportPricesPage() {
  ...
  const [filter, setFilter] = useState<"all" | "changed" | "unchanged">(
    "changed",
  );
    ...
    <div className="admin-page-frame admin-page-frame-fluid">
      ...
      <div className="bg-card border border-border rounded-2xl p-6 mb-6">
        ...
        <ol className="text-sm text-muted-foreground space-y-1.5 list-decimal list-inside">
          {" "}
          <li>
            Скачай прайс из Google Таблицы как CSV (Файл → Скачать → CSV) или
            скопируй содержимое
          </li>{" "}
          ...
          <li>
            ...
            — увидишь что обновится, что не найдено
          </li>{" "}
          <li>
            Если всё ок — нажми{" "}
            <span className="text-foreground font-medium">«Применить»</span>{" "}
            (обновит цены в БД)
          </li>{" "}
        </ol>{" "}
        <div className="mt-4 text-xs text-muted-foreground bg-muted/40 rounded-xl p-3 border border-border/40">
          {" "}
          <b>Формат CSV:</b> заголовок секции (например «Обрезная доска 1 сорт
          ГОСТ»), затем строка с колонками (Сечение | Длина | Кол-во шт. в м³ |
          Цена за м3 | Цена за шт), затем строки с данными. Может быть много
          секций подряд.{" "}
        </div>{" "}
      </div>{" "}
      {/* Ввод CSV */}{" "}
      <div className="bg-card border border-border rounded-2xl p-6 mb-6">
        ...
        <div className="flex items-center justify-between mb-3">
          ...
          <label className="inline-flex items-center gap-2 text-sm px-3 py-2 rounded-xl border border-border hover:bg-primary/5 cursor-pointer transition-colors">
            {" "}
            <Upload className="w-4 h-4" /> <span>Загрузить файл</span>{" "}
            <input
              type="file"
              accept=".csv,.txt"
...
...`}
        ...
        <div className="flex items-center justify-between mt-4">
          ...
          <div className="text-xs text-muted-foreground">
            {" "}
            {csv.length > 0
              ? `${csv.length.toLocaleString("ru-RU")} символов · ${csv.split("\n").length} строк`
              : "Пусто"}{" "}
          </div>{" "}
          <button
            onClick={preview}
        ...
        <>
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-3 mb-6">
            ...
            <StatCard
              ...
              color={report.unmatched > 0 ? "amber" : undefined}
            />{" "}
          </div>{" "}
          {/* Unmatched */}{" "}
          {unmatched.length > 0 && (
            <div className="bg-amber-500/5 border border-amber-500/30 rounded-2xl p-5 mb-6">
              ...
              <div className="text-xs text-muted-foreground mb-3">
                {" "}
                Эти строки из CSV не удалось связать с товарами в базе. Цены для
                ...
                <div className="flex gap-1 bg-muted/40 rounded-xl p-1">
                  ...
                  {(["changed", "unchanged", "all"] as const).map((f) => (
                    <button
                      key={f}
                      onClick={() => setFilter(f)}
                      className={`px-3 py-1.5 text-xs rounded-xl transition-colors ${filter === f ? "bg-primary text-primary-foreground" : "hover:bg-primary/10"}`}
                    >
                      {" "}
                      {f === "changed"
                        ? `Изменится (${matched.filter((m) => m.changed).length})`
                        : f === "unchanged"
                          ? `Без изменений (${matched.filter((m) => !m.changed).length})`
                          : `Все (${matched.length})`}{" "}
                    </button>
                  ))}{" "}
                </div>{" "}
                  ...
                  <tbody>
                    ...
                    {visibleMatched.map((m, i) => {
                      const cubeChanged =
                        ...
                        m.newPricePerCube !== m.oldPricePerCube;
                      const pieceChanged =
                        m.newPricePerPiece !== null &&
                        m.newPricePerPiece !== m.oldPricePerPiece;
                      return (
                        <tr
                          key={i}
          ...
          {report.changed > 0 && (
            <div className="sticky bottom-4 z-20 bg-card border border-primary/30 rounded-2xl p-4 flex items-center justify-between gap-4">
              ...
              <div>
                ...
                <div className="text-xs text-muted-foreground">
                  {" "}
                  Цены будут обновлены в базе. Действие нельзя откатить (только
                  повторным импортом старых цен).{" "}
                </div>{" "}
              </div>{" "}
              {!confirm ? (
                <button
          ...
          {report.changed === 0 && report.matched > 0 && (
            <div className="bg-muted/40 border border-border rounded-2xl p-4 text-center text-sm text-muted-foreground">
              {" "}
              Товары сопоставлены, но цены в CSV совпадают с базой — применять
              нечего.{" "}
            </div>
          )}{" "}
        </>
