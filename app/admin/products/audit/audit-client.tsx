...
const AUDIT_TONE_STYLE = {
  "--background": "220 14% 7%",
  "--foreground": "214 22% 93%",
  "--card": "220 12% 10%",
  "--popover": "220 12% 10%",
  "--muted": "220 10% 15%",
  "--muted-foreground": "216 12% 67%",
  "--border": "218 14% 22%",
  "--input": "218 14% 22%",
  "--primary": "210 86% 62%",
...
export function AuditClient({ products, emptyCategories }: Props) {
  ...
  return (
    <div className="admin-page-frame admin-page-frame-fluid rounded-2xl bg-background text-foreground" style={AUDIT_TONE_STYLE}>
      {/* Сводка */}
      ...
      {totalProblems === 0 && (
        <div className="rounded-2xl border border-border bg-card p-6 flex items-start gap-4">
          ...
          <div>
            <div className="text-lg font-semibold text-foreground">Каталог чистый</div>
        ...
        <ProblemSection
          ...
          action={
            <button
              ...
              className="inline-flex items-center gap-2 rounded-xl border border-border bg-background px-4 py-2 text-sm font-medium text-foreground transition-colors hover:border-primary/35 hover:bg-primary/[0.06] disabled:opacity-50"
            >
        ...
        >
          <Table
            ...
            rows={checks.variantsNoPrice.map((v) => [
              ...
              "—",
              v.variant.inStock ? (
                <span key="is" className="text-foreground text-xs">да</span>
              ) : (
        ...
        >
          <Table
            ...
            rows={checks.productsNoImage.map((p) => [
              ...
              p.variants.length,
              p.active ? (
                <span key="a" className="text-foreground text-xs">да</span>
              ) : (
...
function StatCard({
  ...
  const colorClass =
    color === "emerald" ? "text-primary" :
    color === "amber" ? "text-foreground" :
    "text-foreground";
  return (
...
function ProblemSection({
  ...
  const bgClass =
    color === "red" ? "bg-card border-border" :
    color === "amber" ? "bg-card border-border" :
...
function RowAction({
  ...
  const toneClass =
    tone === "emerald"
      ? "border-border bg-background text-foreground hover:border-primary/35 hover:bg-primary/[0.06]"
      : tone === "red"
      ? "border-destructive/35 bg-background text-destructive hover:bg-destructive/10"
      : "border-border bg-background text-foreground hover:border-primary/35 hover:bg-primary/[0.06]";
  return (
    <button
      onClick={onClick}
...
function Table({ headers, rows }: { headers: string[]; rows: (React.ReactNode[])[] }) {
  return (
    <div className="max-h-80 overflow-auto rounded-xl border border-border bg-background/40">
