...
export default function WatermarkRecoveryPage() {
  ...
  const [loading, setLoading] = useState(true); // Smart auto-restore state
  const [preview, setPreview] = useState<{
    matched: MatchItem[];
    unmatched: { productId: string; productName: string }[];
  } | null>(null);
  const [previewLoading, setPreviewLoading] = useState(false);
  const [restoring, setRestoring] = useState(false);
  const [restoreResult, setRestoreResult] = useState<{
    restored: number;
    unmatched: { productName: string }[];
  } | null>(null); // Manual matching state
  ...
  function toggleImage(productId: string, imgUrl: string) {
    setAssignments((prev) => {
      ...
      const next = curr.includes(imgUrl)
        ? curr.filter((u) => u !== imgUrl)
        : [...curr, imgUrl];
    ...
    <div className="admin-page-frame admin-page-frame-readable">
      ...
      <div>
        ...
        <h1 className="text-2xl font-display font-bold">
          Восстановление оригинальных фото
        </h1>{" "}
        <p className="text-sm text-muted-foreground mt-1">
          {" "}
          Найдено <strong>{orphanedOriginals.length}</strong> оригинальных
          файлов и <strong>{needsRestore.length}</strong> товаров с водяными
          знаками.{" "}
        </p>{" "}
      </div>{" "}
      {/* ══ SUCCESS ══════════════════════════════════════════════════════════ */}{" "}
      {restoreResult && (
        <div className="bg-emerald-500/10 border border-emerald-500/30 rounded-2xl p-6 space-y-3">
          ...
          <div className="flex items-center gap-2 text-emerald-700 font-bold text-lg">
            {" "}
            <CheckCircle2 className="w-6 h-6" /> Восстановлено{" "}
            {restoreResult.restored} товаров!{" "}
          </div>{" "}
          {restoreResult.unmatched.length > 0 && (
            <div>
              ...
              <p className="text-sm text-muted-foreground mb-1">
                ...
                {restoreResult.unmatched.length}) — назначьте вручную ниже:
              </p>{" "}
              <ul className="text-sm space-y-0.5">
                {" "}
                {restoreResult.unmatched.map((u, i) => (
            ...
            <div className="space-y-4">
              ...
              <div className="rounded-xl bg-muted/50 p-4 space-y-2 max-h-64 overflow-y-auto">
                ...
                <p className="text-sm font-medium text-emerald-700">
                  ✓ Найдено совпадений: {preview.matched.length}
                </p>{" "}
                {preview.matched.map((m) => (
                  ...
                  >
                    ...
                    <span className="text-muted-foreground">
                      → {m.images.map((u) => u.split("/").pop()).join(", ")}
                    </span>{" "}
                  </div>
                ))}{" "}
                {preview.unmatched.length > 0 && (
                  <>
                    ...
                    <p className="text-sm font-medium text-amber-600 mt-2">
                      ⚠ Не найдено совпадений: {preview.unmatched.length}
                    </p>{" "}
              ...
              <div className="flex gap-3 flex-wrap">
                ...
                <button
                  ...
                  className="flex items-center gap-2 px-6 py-3 rounded-xl bg-primary text-primary-foreground font-bold hover:bg-primary/90 disabled:opacity-50 transition-colors "
                >
                  {" "}
                  {restoring ? (
                    <Loader2 className="w-4 h-4 animate-spin" />
                  ...
                  {restoring
                    ? "Восстанавливаем..."
                    : `Применить (${preview.matched.length} товаров)`}{" "}
                </button>{" "}
                <button
                  onClick={() => setPreview(null)}
                  className="px-4 py-3 rounded-xl border border-border text-sm hover:bg-primary/[0.08] transition-colors"
                >
                  {" "}
                  Отмена{" "}
                </button>{" "}
              ...
              {needsRestore.map((product) => {
                ...
                return (
                  <div
                    ...
                    className={`flex items-center gap-3 p-3 rounded-xl border cursor-pointer transition-all ${isSelected ? "border-primary bg-primary/15 " : isSaved ? "border-emerald-500/40 bg-emerald-500/5" : "border-border hover:border-primary/40 bg-card"}`}
                  >
                    {" "}
                    <div className="w-12 h-12 rounded-xl overflow-hidden bg-muted shrink-0 relative">
                      {" "}
                      {product.images[0] && (
                        <Image
                    ...
                    {isSaved ? (
                      ...
                      <button
                        ...
                        className="text-xs px-2 py-1 rounded-xl bg-primary text-primary-foreground shrink-0"
                      >
                        {" "}
                        {saving === product.id ? (
                ...
                {orphanedOriginals.map((url) => {
                  ...
                  return (
                    <div
                      key={url}
                      onClick={() => {
                        if (selected) toggleImage(selected, url);
                      }}
                      className={`relative aspect-square rounded-xl overflow-hidden border-2 transition-all cursor-pointer ${isChosen ? "border-primary ring-2 ring-primary/30" : selected ? "border-border hover:border-primary/60" : "border-border opacity-60 cursor-default"}`}
                    >
                      {" "}
                      <Image
                        src={url}
      ...
      {assignedCount > 0 && (
        <div className="sticky bottom-4 flex justify-end">
          ...
          <button
            onClick={async () => {
              setSaving("__all__");
              const toSave = Object.entries(assignments).filter(
                ([, imgs]) => imgs.length > 0,
              );
              await fetch("/api/admin/watermark-recovery", {
                ...
                body: JSON.stringify({
                  action: "auto_restore",
                  assignments: toSave.map(([productId, images]) => ({
                    productId,
                    images,
                  })),
                }),
            ...
            className="flex items-center gap-2 px-6 py-3 rounded-2xl bg-primary text-primary-foreground font-semibold hover:bg-primary/90 disabled:opacity-50"
          >
            {" "}
            {saving === "__all__" ? (
              <Loader2 className="w-4 h-4 animate-spin" />
