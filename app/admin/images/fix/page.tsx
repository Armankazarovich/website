...
export default function ImageFixPage() {
  ...
  const doFixAction = async (action: string) => {
    ...
    try {
      ...
      if (json.ok) {
        setResult(
          `${json.fixed} товаров исправлено, удалено ${json.totalRemoved} записей`,
        );
        setResultOk(true);
        await scan(); // refresh
      } else {
    ...
    <div className="admin-page-frame admin-page-frame-readable">
      ...
      <div className="flex items-center gap-3 mb-2">
        ...
        <div>
          ...
          <h1 className="font-display font-bold text-2xl flex items-center gap-2">
            {" "}
            <ScanSearch className="w-6 h-6 text-primary" /> Диагностика
            фотографий{" "}
          </h1>{" "}
      ...
      {s && (
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          ...
          >
            ...
            {s.totalDuplicateEntries > 0 && (
              <p className="text-xs text-amber-500 font-medium">
                {s.totalDuplicateEntries} записей
              </p>
            )}{" "}
          </div>{" "}
          <div
          ...
          >
            {" "}
            <p className="text-2xl font-bold font-display text-slate-400">
              {s.withNoImages}
            </p>{" "}
      ...
      {result && (
        <div
          className={`px-4 py-3 rounded-xl text-sm font-medium flex items-center gap-2 ${resultOk === true ? "bg-emerald-50 dark:bg-emerald-950/30 text-emerald-700 dark:text-emerald-400 border border-emerald-200 dark:border-emerald-800" : "bg-red-50 dark:bg-red-950/30 text-red-700 dark:text-red-400 border border-red-200 dark:border-red-800"}`}
        >
          {" "}
          {resultOk === true ? (
        ...
        (s.withDuplicates > 0 ||
          ...
          <div className="bg-card border border-border rounded-2xl p-5 space-y-3">
            ...
            <h2 className="font-semibold flex items-center gap-2">
              {" "}
              <Wrench className="w-4 h-4 text-primary" /> Автоматическое
              исправление{" "}
            </h2>{" "}
            <div className="grid sm:grid-cols-3 gap-3">
              ...
              {s.withDuplicates > 0 && (
                <button
                  onClick={() =>
                    fixAction(
                      "deduplicate",
                      `Удалить ${s.totalDuplicateEntries} дублирующихся записей из ${s.withDuplicates} товаров`,
                    )
                  }
                  disabled={!!fixing}
                ...
                >
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground mt-0.5">
                      {" "}
                      {s.totalDuplicateEntries} лишних записей в{" "}
                      {s.withDuplicates} товарах{" "}
                    </p>{" "}
                  </div>{" "}
                </button>
              )}{" "}
              {s.withBroken > 0 && (
                <button
                  onClick={() =>
                    fixAction(
                      "remove_broken",
                      `Удалить ${s.totalBrokenRefs} ссылок на несуществующие файлы`,
                    )
                  }
                  disabled={!!fixing}
              ...
              {(s.withRestorableNoImages ?? 0) > 0 && (
                <button
                  onClick={() =>
                    fixAction(
                      "fill_missing_by_slug",
                      `Заполнить фото у ${s.withRestorableNoImages} товаров по точному имени файла`,
                    )
                  }
                  disabled={!!fixing}
      ...
      <div className="bg-primary/15 border border-primary/20 rounded-2xl p-5">
        ...
        <h2 className="font-semibold flex items-center gap-2 mb-3">
          {" "}
          <CheckCircle2 className="w-4 h-4 text-primary" /> Восстановление из
          резервной копии{" "}
        </h2>{" "}
        <p className="text-sm text-muted-foreground mb-4">
          {" "}
          Если водяной знак был применён и хотите вернуть оригинальные URL фото
          — используйте кнопку восстановления. Резервная копия создаётся
          автоматически перед каждым применением водяного знака.{" "}
        </p>{" "}
        <div className="flex gap-3 flex-wrap">
          ...
          >
            {" "}
            <RefreshCw className="w-4 h-4" /> Перейти к водяному знаку →
            Восстановить{" "}
          </Link>{" "}
          <Link
            href="/admin/media"
          ...
          {filtered.map((p) => (
            ...
            >
              ...
              <div className="flex gap-2 shrink-0">
                {" "}
                {p.total === 0 && (
                  <span className="inline-flex items-center gap-1 px-2 py-1 rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-500 text-xs font-medium">
                    {" "}
                ...
                {p.hasDuplicates && (
                  <span className="inline-flex items-center gap-1 px-2 py-1 rounded-xl bg-amber-100 dark:bg-amber-900/30 text-amber-600 dark:text-amber-400 text-xs font-medium">
                    {" "}
                    <Layers className="w-3 h-3" /> {p.duplicatesCount} дубл
                    {p.duplicatesCount === 1 ? "" : "я"}{" "}
                  </span>
                )}{" "}
                {p.hasBroken && (
                ...
                <Link
                  href={`/admin/products/${p.id}`}
                  className="px-3 py-1 rounded-xl border border-border bg-muted text-xs font-medium hover:bg-primary hover:text-primary-foreground hover:border-primary transition-colors"
                >
                  {" "}
                  Открыть{" "}
                </Link>{" "}
