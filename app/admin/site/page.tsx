...
export default function AdminSitePage() {
  ...
  const [activeTab, setActiveTab] = useState<"contacts" | "company" | "seo" | "analytics" | "promo" | "messengers" | "footer" | "widget">("contacts");
        ...
        {activeTab === "messengers" && (
          <>
            <h2 className="font-semibold flex items-center gap-2">
              <MessageCircle className="w-4 h-4 text-primary" /> Кнопки заказа — Мессенджеры
            </h2>
            <p className="text-sm text-muted-foreground">Кнопки появляются на странице товара. Включите нужные и настройте номер/юзернейм.</p>
            ...
            <div className="p-5 bg-card border border-border rounded-2xl space-y-4">
              <div className="flex items-center justify-between">
                ...
                <button
                  onClick={() => set("whatsapp_enabled", settings["whatsapp_enabled"] === "true" ? "false" : "true")}
                  className={`relative w-11 h-6 rounded-full transition-colors ${settings["whatsapp_enabled"] === "true" ? "bg-[#25D366]" : "bg-muted"}`}
                >
                  <span className={`absolute top-0.5 w-5 h-5 bg-white rounded-full shadow transition-transform ${settings["whatsapp_enabled"] === "true" ? "translate-x-5.5 left-0.5" : "left-0.5"}`} />
                </button>
              </div>
              {settings["whatsapp_enabled"] === "true" && (
                <div className="space-y-3 pt-2 border-t border-border">
                  <div>
                    ...
                    <input
                      value={settings["whatsapp_number"] ?? "+79859707133"}
                  ...
                  <div>
                    <label className="text-xs font-medium text-muted-foreground uppercase tracking-wide mb-1 block">Текст сообщения по умолчанию</label>
                    <input
                      value={settings["whatsapp_message"] ?? "Здравствуйте! Хочу сделать заказ."}
                      onChange={(e) => set("whatsapp_message", e.target.value)}
                      placeholder="Здравствуйте! Хочу сделать заказ."
                      className="w-full px-3 py-2 rounded-xl border border-border bg-background text-sm focus:outline-none focus:ring-2 focus:ring-primary/30"
                    />
                  </div>
                  <div className="p-3 bg-[#25D366]/5 border border-[#25D366]/20 rounded-xl text-xs text-muted-foreground">
                    Превью: <span className="text-foreground font-medium">wa.me/{(settings["whatsapp_number"] ?? "+79859707133").replace(/\D/g, "")}</span>
                  </div>
                </div>
              )}
            </div>
            ...
            <div className="p-5 bg-card border border-border rounded-2xl space-y-4">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-xl bg-[#2AABEE]/15 flex items-center justify-center">
                ...
                <button
                  onClick={() => set("telegram_enabled", settings["telegram_enabled"] === "true" ? "false" : "true")}
                  className={`relative w-11 h-6 rounded-full transition-colors ${settings["telegram_enabled"] === "true" ? "bg-[#2AABEE]" : "bg-muted"}`}
                >
                  <span className={`absolute top-0.5 w-5 h-5 bg-white rounded-full shadow transition-transform ${settings["telegram_enabled"] === "true" ? "translate-x-5.5 left-0.5" : "left-0.5"}`} />
                </button>
              </div>
                ...
                <div className="space-y-3 pt-2 border-t border-border">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full px-3 py-2 rounded-xl border border-border bg-background text-sm focus:outline-none focus:ring-2 focus:ring-primary/30"
                    />
                    <p className="text-xs text-muted-foreground mt-1">Например: @pilorus_orders_bot или ваш личный аккаунт</p>
                  </div>
                  <div>
                    <label className="text-xs font-medium text-muted-foreground uppercase tracking-wide mb-1 block">Текст сообщения по умолчанию</label>
                    <input
                      value={settings["telegram_message"] ?? "Здравствуйте! Хочу сделать заказ."}
                      onChange={(e) => set("telegram_message", e.target.value)}
                      placeholder="Здравствуйте! Хочу сделать заказ."
                      className="w-full px-3 py-2 rounded-xl border border-border bg-background text-sm focus:outline-none focus:ring-2 focus:ring-primary/30"
                    />
                  </div>
            ...
            <div className="p-3 bg-muted/40 rounded-xl text-xs text-muted-foreground">
              💾 Не забудьте нажать <strong>Сохранить</strong> после изменений
            </div>
          </>
        )}
