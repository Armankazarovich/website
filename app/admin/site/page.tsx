...
function Toggle({
  ...
  return (
    ...
    >
      <span
        className={`absolute top-0.5 w-5 h-5 bg-white rounded-full shadow transition-all ${value ? "left-[22px]" : "left-0.5"}`}
      />
    </button>
  );
}
...
function Card({
  ...
  return (
    <div
      className={`p-4 bg-card border border-border rounded-2xl space-y-3 ${className}`}
    >
      {children}
    </div>
  );
    ...
    <div>
      ...
      {rows ? (
        <textarea
          ...
          className="w-full min-h-11 px-3 py-2 rounded-xl border border-border bg-background text-sm focus:outline-none focus:ring-2 focus:ring-primary/30 resize-y"
        />
      ) : (
        <input
          type={type}
...
export default function AdminSitePage() {
  ...
  const [activeTab, setActiveTab] = useState<
    "contacts" | "company" | "seo" | "analytics" | "promo" | "widget" | "footer"
  >("contacts");
  const [pinging, setPinging] = useState(false);
  const [pingResults, setPingResults] = useState<
    { name: string; status: string; ms: number }[] | null
  >(null);
  ...
  useEffect(() => {
    ...
    async function loadSettings() {
      ...
      try {
        ...
        if (!cancelled)
          setLoadError(err?.message || "Не удалось загрузить настройки сайта");
      } finally {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (Array.isArray(data.rejected) && data.rejected.length > 0) {
        throw new Error(`Не сохранены поля: ${data.rejected.join(", ")}`);
      }
      setSaved(true);
      setTimeout(() => setSaved(false), 2500);
    } catch (err: any) {
      setSaveError(err?.message || "Не удалось сохранить настройки сайта");
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <SettingsCtx.Provider value={{ settings, set }}>
      <div className="admin-page-frame admin-page-frame-fluid space-y-6">
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="min-w-0">
            ...
            <p className="text-sm text-muted-foreground mt-1">
              Контакты, SEO, мессенджеры, продвижение — всё в одном месте
            </p>
          </div>
          <Button
            onClick={handleSave}
        ...
        {/* Tabs */}
        <div className="flex gap-1 overflow-x-auto rounded-xl bg-muted/50 p-1 sm:flex-wrap">
          {tabs.map((tab) => (
            <button
              key={tab.id}
              onClick={() => setActiveTab(tab.id)}
              className={`flex min-h-10 flex-none items-center gap-1.5 whitespace-nowrap rounded-lg px-3 py-2 text-sm font-medium transition-colors ${
                activeTab === tab.id
        ...
        {activeTab === "contacts" && (
          <div className="bg-card rounded-2xl border border-border p-6 space-y-5">
            <SectionHeader
              ...
              desc="Телефоны, адрес, часы — отображаются по всему сайту"
            />
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <Field
                ...
                settingKey="phone2"
                placeholder="пусто или новый номер"
              />
              <Field
                label="Телефон 2 (для tel: ссылки)"
                settingKey="phone2_link"
              ...
              <Field
                ...
                placeholder="+79776068020"
              />
              <Field
                label="Email"
                settingKey="email"
                placeholder="info@pilo-rus.ru"
                type="email"
              />
              <Field
                label="Минимальный заказ"
                settingKey="min_order"
                placeholder="1 м³"
              />
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <Field
                ...
                settingKey="address_map"
                placeholder="https://yandex.ru/maps/..."
              />
              <Field
                label="Город компании (для SEO)"
                settingKey="company_city"
                placeholder="Химки"
                hint="Используется в автоописаниях товаров: «от производителя в Химках»"
              />
              <Field
                label="Регион доставки (для SEO)"
                settingKey="delivery_region"
            ...
            <Field
              ...
              placeholder="Пн–Пт: 09:00–18:00, Сб: 09:00–15:00"
            />

            <Divider />
            <SubHeader
              icon={MessageCircle}
              title="Страница «Контакты» — дополнительный текст"
            />
            <Field
              label="Описание под заголовком"
              settingKey="contacts_description"
              rows={3}
        ...
        {activeTab === "company" && (
          <div className="bg-card rounded-2xl border border-border p-6 space-y-5">
            <SectionHeader
              ...
              desc="Реквизиты, тексты страниц О нас и Доставка"
            />
        ...
        {activeTab === "seo" && (
          <div className="bg-card rounded-2xl border border-border p-6 space-y-5">
            ...
            <Field
              ...
              placeholder="ПилоРус — пиломатериалы от производителя в Химках"
              hint={`Рекомендуется 50–60 символов. Текущая длина: ${(settings["seo_title"] || "").length}`}
            />

            <Field
              label="Meta Description (описание в Google)"
            ...
            <p className="text-xs text-muted-foreground">
              Получите код → вставьте сюда → сохраните → сайт подтверждён.
            </p>
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  <label className="text-xs font-medium text-muted-foreground uppercase tracking-wide flex items-center gap-1.5">
                    <span className="w-4 h-4 rounded bg-amber-500 flex items-center justify-center text-white text-[9px] font-bold">
                      Я
                    </span>
                    Яндекс Вебмастер
                  </label>
                ...
                <input
                  ...
                  onChange={(e) => set("yandex_verification", e.target.value)}
                  placeholder="yandex_12ab34cd56ef78gh"
                  className="w-full min-h-11 px-3 py-2 rounded-xl border border-border bg-background text-sm focus:outline-none focus:ring-2 focus:ring-primary/30"
                />
                <p className="text-xs text-muted-foreground">
                  Вебмастер → Добавить сайт → HTML-тег → content="..."
                </p>
              </div>
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  <label className="text-xs font-medium text-muted-foreground uppercase tracking-wide flex items-center gap-1.5">
                    <span className="w-4 h-4 rounded bg-blue-500 flex items-center justify-center text-white text-[9px] font-bold">
                      G
                    </span>
                ...
                />
                <p className="text-xs text-muted-foreground">
                  Search Console → Добавить ресурс → HTML-тег → content="..."
                </p>
              </div>
            </div>
          </div>
        ...
        {activeTab === "analytics" && (
          <div className="bg-card rounded-2xl border border-border p-6 space-y-5">
            ...
            <Card>
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <span className="w-7 h-7 rounded-lg bg-amber-500 flex items-center justify-center text-white text-xs font-bold shrink-0">
                    Я
                  </span>
                  <div>
                    <p className="text-sm font-semibold">Яндекс Метрика</p>
                    <p className="text-xs text-muted-foreground">
                      Подробная аналитика для российских сайтов
                    </p>
                  </div>
                </div>
                <div className="flex items-center gap-2">
              ...
              <input
                ...
                onChange={(e) => set("yandex_metrika_id", e.target.value)}
                placeholder="Номер счётчика, например: 98765432"
                className="w-full min-h-11 px-3 py-2 rounded-xl border border-border bg-background text-sm focus:outline-none focus:ring-2 focus:ring-primary/30"
              />
              <p className="text-xs text-muted-foreground -mt-1">
                Яндекс Метрика → Настройки → Номер счётчика
              </p>
            </Card>
            ...
            <Card>
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <span className="w-7 h-7 rounded-lg bg-blue-500 flex items-center justify-center text-white text-xs font-bold shrink-0">
                    G
                  </span>
                  <div>
                    <p className="text-sm font-semibold">Google Analytics 4</p>
                    <p className="text-xs text-muted-foreground">
                      Международная аналитика, интеграция с Google Ads
                    </p>
                  </div>
                </div>
                <div className="flex items-center gap-2">
            ...
            <p className="text-xs text-muted-foreground">
              Отправляет sitemap сразу в Google, Яндекс и Bing — сервер делает
              это напрямую без CORS ограничений браузера.
            </p>

            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 p-3 bg-muted/30 rounded-xl border border-border text-xs">
            ...
            >
              {pinging ? (
                <>
                  <Loader2 className="w-4 h-4 animate-spin" /> Пингуем 5
                  источников...
                </>
              ) : (
                <>
                  <Radio className="w-4 h-4" /> Отправить в Google + Яндекс +
                  Bing + RSS
                </>
              )}
            </button>
            ...
            {pingResults && (
              <div className="space-y-1.5">
                {pingResults.map((r) => (
                  <div
                    key={r.name}
                    className="flex items-center justify-between px-3 py-2 bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-800/40 rounded-xl"
                  >
                    <div className="flex items-center gap-2">
          ...
          <div className="space-y-4">
            ...
            <div className="bg-card rounded-2xl border border-border p-5">
              ...
              <div className="flex items-start gap-3 p-3.5 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800/40 rounded-xl text-xs text-amber-800 dark:text-amber-300">
                ...
                <p>
                  <strong>Главный совет:</strong> Яндекс Бизнес + Google Бизнес
                  дают больше клиентов чем всё остальное вместе. Регистрация
                  бесплатная, эффект через 2–4 недели.
                </p>
              </div>
            </div>
            ...
            <Card>
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-xs text-muted-foreground">
                    Яндекс.Новости и агрегаторы автоматически подхватывают новые
                    статьи
                  </p>
                </div>
              ...
              <div className="flex gap-2">
                <a
                  href="https://pilo-rus.ru/rss.xml"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="flex-1 flex min-h-11 items-center justify-center gap-1.5 rounded-xl border border-border px-3 py-2 text-xs font-medium hover:bg-primary/10 hover:text-primary transition-colors"
                >
                  <ExternalLink className="w-3.5 h-3.5" /> Открыть RSS
                </a>
                <a
                  href="https://news.yandex.ru/addmedia.html"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="flex-1 flex min-h-11 items-center justify-center gap-1.5 rounded-xl border border-border px-3 py-2 text-xs font-medium hover:bg-primary/10 hover:text-primary transition-colors"
                >
                  <ExternalLink className="w-3.5 h-3.5" /> Подать в
                  Яндекс.Новости
                </a>
              </div>
            </Card>
            ...
            <Card>
              <div className="flex items-center gap-3 mb-1">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">
                    Мощнейшие бесплатные источники трафика
                  </p>
                </div>
              </div>
              <div className="space-y-2">
                {[
                  {
                    name: "Яндекс Бизнес",
                    desc: "Карточка в картах + поиске Яндекса",
                  ...
                  {
                    name: "Google Бизнес-профиль",
                    desc: "Google Maps + поиск Google",
                  ...
                  {
                    name: "2ГИС",
                    desc: "Популярный в регионах РФ",
                    href: "https://2gis.ru/firms",
                    priority: "Важно",
                    color:
                  ...
                  {
                    name: "Авито — Компании",
                    desc: "Тысячи просмотров бесплатно",
                    href: "https://www.avito.ru/profile/profile",
                    priority: "Хорошо",
                    color:
                  ...
                  >
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      <span
                        className={`text-[10px] px-2 py-0.5 rounded-full font-medium ${item.color}`}
                      >
                        {item.priority}
                      </span>
            ...
            <Card>
              <div className="flex items-center gap-3 mb-1">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">
                    Разместите товары — получите трафик из поиска
                  </p>
                </div>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                {[
                  ...
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex min-h-11 items-center justify-between gap-2 rounded-xl border border-border p-2.5 text-sm font-medium transition-all hover:border-primary/30 hover:bg-primary/5"
                  >
                    {name}{" "}
            ...
            <Card>
              <div className="flex items-center gap-3 mb-1">
                ...
                <div>
                  <p className="text-sm font-semibold">
                    Отраслевые каталоги строительства
                  </p>
                  <p className="text-xs text-muted-foreground">
                    Ссылочная масса и целевой трафик
                  </p>
                </div>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                {[
                  ...
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex min-h-11 items-center justify-between gap-2 rounded-xl border border-border p-2.5 text-xs font-medium transition-all hover:border-primary/30 hover:bg-primary/5"
                  >
                    {name}{" "}
                    <ExternalLink className="w-3 h-3 text-muted-foreground" />
                  </a>
            ...
            <div className="bg-card rounded-2xl border border-border p-5 space-y-4">
              <SectionHeader
                icon={MessageCircle}
                title="Кнопки заказа — мессенджеры"
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">
                        Кнопка «Заказать через WhatsApp» на странице товара
                      </p>
                    </div>
                  </div>
                  <Toggle
                    ...
                    onChange={(v) =>
                      set("whatsapp_enabled", v ? "true" : "false")
                    }
                    color="bg-[#25D366]"
                  />
                </div>
                  ...
                  <div className="space-y-3 pt-3 border-t border-border">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      <div>
                        ...
                        <input
                          value={settings["whatsapp_number"] ?? "+79850670888"}
                          onChange={(e) =>
                      ...
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            set("whatsapp_message", e.target.value)
                          }
                    ...
                    <div className="p-2.5 bg-[#25D366]/5 border border-[#25D366]/20 rounded-xl text-xs text-muted-foreground">
                      ...
                      <span className="text-foreground font-mono">
                        wa.me/
                        {(
                          settings["whatsapp_number"] ?? "+79850670888"
                        ).replace(/\D/g, "")}
                      </span>
                    </div>
                  </div>
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">
                        Кнопка «Написать в Telegram» на странице товара
                      </p>
                    </div>
                  </div>
                  <Toggle
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            set("telegram_username", e.target.value)
                          }
                          placeholder="@pilorus или t.me/pilorus"
                          className="w-full min-h-11 px-3 py-2 rounded-xl border border-border bg-background text-sm focus:outline-none focus:ring-2 focus:ring-primary/30"
                        />
                        <p className="text-xs text-muted-foreground mt-1">
                          Например: @pilorus_orders_bot
                        </p>
                      </div>
                      <div>
                        <label className="text-xs font-medium text-muted-foreground uppercase tracking-wide mb-1 block">
                        ...
                        <input
                          value={
                            settings["telegram_message"] ??
                            "Здравствуйте! Хочу сделать заказ."
                          }
                          onChange={(e) =>
                            set("telegram_message", e.target.value)
                          }
                          placeholder="Здравствуйте! Хочу сделать заказ."
                          className="w-full min-h-11 px-3 py-2 rounded-xl border border-border bg-background text-sm focus:outline-none focus:ring-2 focus:ring-primary/30"
                        />
              ...
              <div className="flex items-center gap-2 p-3 bg-muted/40 rounded-xl text-xs text-muted-foreground">
                ...
                Не забудьте нажать{" "}
                <strong className="text-foreground mx-1">Сохранить</strong> в
                верхнем углу после изменений
              </div>
            </div>
          </div>
        )}
        ...
        {activeTab === "widget" && (
          <div className="bg-card rounded-2xl border border-border p-6 space-y-5">
                ...
                <div className="flex gap-2">
                  {[
                    ...
                    <button
                      ...
                      className={`flex-1 min-h-11 rounded-xl border px-3 py-2 text-sm font-medium transition-colors ${
            ...
            <p className="text-xs text-muted-foreground -mt-3">
              Заполненные поля автоматически появляются в виджете
            </p>
            ...
            <div className="flex items-center gap-3 p-3 bg-emerald-50 dark:bg-emerald-950/20 border border-emerald-200 dark:border-emerald-800 rounded-xl">
              ...
              <span className="text-xs bg-emerald-100 dark:bg-emerald-900 text-emerald-700 dark:text-emerald-400 px-2 py-0.5 rounded-full shrink-0">
                Всегда активен
              </span>
            </div>
              ...
              >
                ...
                <div className="flex-1">
                  ...
                  <input
                    ...
                    placeholder={placeholder}
                    className="mt-1 w-full min-h-11 px-2.5 py-1.5 rounded-lg border border-border bg-background text-sm focus:outline-none focus:ring-2 focus:ring-primary/30"
                  />
                </div>
              </div>
            ))}
            ...
            <div className="flex items-center gap-3 p-3 bg-card border border-border rounded-xl">
              ...
              <div className="flex-1">
                ...
                <p className="text-xs text-muted-foreground">
                  {settings["email"] || "Настройте в разделе «Контакты»"}
                </p>
              </div>
              <button
                type="button"
                onClick={() =>
                  set(
                    "widget_show_email",
                    settings["widget_show_email"] === "true" ? "false" : "true",
                  )
                }
                className={`min-h-11 shrink-0 rounded-xl border px-3 py-1.5 text-xs font-medium transition-colors ${
            ...
            <div className="p-4 bg-muted/30 rounded-xl border border-border">
              <p className="text-xs font-medium text-muted-foreground mb-2.5 uppercase tracking-wide">
                Активные каналы (предпросмотр)
              </p>
              <div className="flex flex-wrap gap-2">
                <span className="flex items-center gap-1.5 px-3 py-1.5 bg-emerald-500 text-white rounded-xl text-xs font-medium">
                  <Phone className="w-3 h-3" /> Телефон
                </span>
                {settings["social_whatsapp"] && (
                  <span className="flex items-center gap-1.5 px-3 py-1.5 bg-[#25D366] text-white rounded-xl text-xs font-medium">
                    <MessageCircle className="w-3 h-3" /> WhatsApp
                  </span>
                )}
                {settings["social_telegram"] && (
                  <span className="flex items-center gap-1.5 px-3 py-1.5 bg-[#2AABEE] text-white rounded-xl text-xs font-medium">
                ...
                {settings["widget_show_email"] === "true" && (
                  <span className="flex items-center gap-1.5 px-3 py-1.5 bg-muted border border-border rounded-xl text-xs font-medium">
        ...
        {activeTab === "footer" && (
          <div className="bg-card rounded-2xl border border-border p-6 space-y-5">
            <SectionHeader
              icon={Settings}
              title="Футер (нижняя часть сайта)"
              desc="Copyright строка в подвале"
            />
            <Field
              label="Строка Copyright"
              settingKey="footer_copyright"
              placeholder={`© ${new Date().getFullYear()} ПилоРус. Все права защищены.`}
            />
            <div className="flex items-start gap-2 p-4 bg-muted/30 rounded-xl border border-border text-sm text-muted-foreground">
              ...
              <span>
                Остальные данные в футере (телефон, адрес, часы) берутся из
                раздела <strong className="text-foreground">«Контакты»</strong>{" "}
