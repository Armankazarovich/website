...
export default function NotificationsPage() {
  const [tab, setTab] = useState<"center" | "send" | "subscribers">("center");
  ...
  const [subsFilter, setSubsFilter] = useState<"all" | "registered" | "guests">(
    "all",
  );
  ...
  const [subscribeResult, setSubscribeResult] = useState<"ok" | "err" | null>(
    null,
  );
  ...
  const checkTelegram = async () => {
    ...
    try {
      ...
      if (!res.ok)
        throw new Error(data.error || "Не удалось проверить Telegram");
      setTgStatus(data);
    } catch (error: any) {
      setTgStatus({
        configured: false,
  ...
  const setupTelegram = async () => {
    ...
    try {
      ...
      const result = res.ok
        ? data
        : { ok: false, error: data.error || "Не удалось настроить вебхук" };
      setTgResult(result);
      if (result.ok) checkTelegram();
    } catch (error: any) {
      setTgResult({
        ok: false,
        error: error.message || "Не удалось настроить вебхук",
      });
    } finally {
      setTgSetting(false);
    }
  ...
  const testTelegram = async () => {
    ...
    try {
      ...
      setTgResult(
        res.ok
          ? data
          : { ok: false, error: data.error || "Тест Telegram не отправлен" },
      );
    } catch (error: any) {
      setTgResult({
        ok: false,
        error: error.message || "Тест Telegram не отправлен",
      });
    } finally {
      setTgTesting(false);
    }
    ...
    <div className="admin-page-frame admin-page-frame-readable">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Push-рассылки, подписчики и Telegram-бот в одном месте
          </p>
        </div>
      </div>
      ...
      <div className="flex gap-1 bg-muted/40 p-1 rounded-xl w-fit">
        <button
          ...
          className={`admin-pill-btn ${tab === "center" ? "admin-pill-btn-active" : ""}`}
        >
          Центр
        </button>
        <button
        ...
        >
          Подписчики{" "}
          {subs.length > 0 && (
            <span className="ml-1 text-xs text-primary">({subs.length})</span>
          )}
        </button>
      </div>
      ...
      {tab === "center" && (
        <div className="space-y-4">
          <NotificationCenterPanel refreshKey={centerRefreshKey} />
        </div>
      )}
        ...
        <div className="space-y-5">
          <div className="bg-muted/30 rounded-xl p-4 text-sm text-muted-foreground space-y-1.5">
            <p className="flex items-center gap-2">
              ...
              <span>
                <strong>Новый заказ:</strong> команда сразу видит заявку, клиент получает подтверждение
              </span>
            </p>
            <p className="flex items-center gap-2">
              <Package className="w-4 h-4 shrink-0" />{" "}
              <span>
                <strong>Статус заказа:</strong> клиенту уходит короткое уведомление об изменении
              </span>
            </p>
            <p className="flex items-center gap-2">
              <Lightbulb className="w-4 h-4 shrink-0" />{" "}
              <span>
                <strong>Плановые сообщения:</strong> советы каждый понедельник, акции по
                пятницам
              </span>
            </p>
            <p className="flex items-center gap-2">
              ...
              <span>
                <strong>Ручная отправка:</strong> выберите группу получателей и отправьте короткое сообщение
              </span>
            </p>
          </div>
          ...
          <div className="bg-card border border-border rounded-2xl p-5 space-y-3">
            <div className="flex items-center justify-between flex-wrap gap-2">
              ...
              <div className="flex flex-wrap gap-2">
                <button
                  ...
                  className="flex min-h-11 items-center gap-1.5 rounded-lg bg-muted/40 px-3 py-2 text-xs font-medium transition-colors hover:bg-primary/[0.08] disabled:opacity-50"
                >
                  {debugLoading ? (
                ...
                <button
                  ...
                  className="flex min-h-11 items-center gap-1.5 rounded-lg bg-primary px-3 py-2 text-xs font-medium text-primary-foreground transition-colors hover:bg-primary/90 disabled:opacity-50"
                >
                  {subscribing ? (
                    <Loader2 className="w-3 h-3 animate-spin" />
                  ) : (
            ...
            {subscribeResult === "err" && (
              <p className="text-xs text-destructive flex items-center gap-1">
                <XCircle className="w-3 h-3" /> Ошибка подписки — проверьте
                консоль
              </p>
            )}
            {debug && (
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 text-xs">
                <div className="bg-muted/40 rounded-lg p-2.5">
                  ...
                  <p className="text-muted-foreground">
                    {debug.withUser} зарег. / {debug.guests} гостей
                  </p>
                </div>
                <div className="bg-muted/40 rounded-lg p-2.5">
                  ...
                  <p
                    className={`font-semibold flex items-center gap-1 ${debug.vapidConfigured ? "text-green-600 dark:text-green-400" : "text-destructive"}`}
                  >
                    {debug.vapidConfigured ? (
                      <>
                        <CheckCircle className="w-3 h-3 shrink-0" /> Настроены
                      </>
                    ) : (
                      <>
                        <XCircle className="w-3 h-3 shrink-0" /> Не настроены
                      </>
                    )}
                  </p>
                  <p className="text-muted-foreground">
                    {debug.publicKeyPrefix}
                  </p>
                </div>
                <div className="bg-muted/40 rounded-lg p-2.5">
                  <p className="text-muted-foreground mb-0.5">Браузер</p>
                  <p
                    className={`font-semibold flex items-center gap-1 ${debug.browserPermission === "granted" ? "text-green-600 dark:text-green-400" : debug.browserPermission === "denied" ? "text-destructive" : "text-yellow-600"}`}
                  >
                    {debug.browserPermission === "granted" ? (
                      <>
                        <CheckCircle className="w-3 h-3 shrink-0" /> Разрешено
                      </>
                    ) : debug.browserPermission === "denied" ? (
                      <>
                        <XCircle className="w-3 h-3 shrink-0" /> Заблокировано
                      </>
                    ) : (
                      <>
                        <AlertTriangle className="w-3 h-3 shrink-0" /> Не
                        спрашивали
                      </>
                    )}
                  </p>
            ...
            {!debug && !debugLoading && (
              <p className="text-xs text-muted-foreground">
                Нажмите «Проверить» чтобы увидеть статус Push системы
              </p>
            )}
          </div>
        ...
        <div className="flex items-center justify-between p-5 border-b border-border">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                Настройка вебхука и проверка соединения
              </p>
            </div>
          </div>
          <button
            ...
            disabled={tgLoading}
            className="flex min-h-11 items-center gap-1.5 rounded-xl border border-border px-3 py-2 text-sm text-muted-foreground transition-colors hover:bg-primary/[0.08] disabled:opacity-50"
          >
            <RefreshCw
              className={`w-3.5 h-3.5 ${tgLoading ? "animate-spin" : ""}`}
            />
            Проверить
          </button>
        </div>
        ...
        <div className="p-5 space-y-4">
          ...
          {tgStatus === null && !tgLoading && (
            <div className="flex items-center gap-3 p-4 rounded-xl bg-muted/50 border border-border">
              ...
              <p className="text-sm text-muted-foreground">
                Нажмите "Проверить" чтобы узнать статус вебхука
              </p>
            </div>
          )}
              ...
              <div className="flex-1 min-w-0">
                <p className="font-medium text-sm flex items-center gap-1.5">
                  {tgStatus.error ? (
                    ...
                    <>
                      <CheckCircle className="w-4 h-4 shrink-0" /> Вебхук
                      настроен корректно
                    </>
                  ) : tgStatus.configured ? (
                    <>
                      <AlertTriangle className="w-4 h-4 shrink-0" /> Вебхук
                      настроен на другой URL
                    </>
                  ) : (
                    <>
                      <XCircle className="w-4 h-4 shrink-0" /> Вебхук не
                      настроен — кнопки в Telegram не работают!
                    </>
                  )}
                </p>
                {tgStatus.webhookUrl && (
                  <p className="text-xs text-muted-foreground mt-1 truncate">
                    URL: {tgStatus.webhookUrl}
                  </p>
                )}
                {tgStatus.pendingUpdateCount !== undefined &&
                  tgStatus.pendingUpdateCount > 0 && (
                    <p className="text-xs text-yellow-600 mt-1 flex items-center gap-1">
                      <AlertTriangle className="w-3 h-3 shrink-0" />{" "}
                      {tgStatus.pendingUpdateCount} необработанных обновлений в
                      очереди
                    </p>
                  )}
                {tgStatus.lastErrorMessage && (
                  <p className="text-xs text-destructive mt-1">
                    Последняя ошибка ({tgStatus.lastErrorDate}):{" "}
                    {tgStatus.lastErrorMessage}
                  </p>
                )}
              </div>
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <button
              ...
              disabled={tgTesting}
              className="flex min-h-11 items-center gap-1.5 rounded-xl border border-border px-4 py-2.5 text-sm font-medium transition-colors hover:bg-primary/[0.08] disabled:opacity-50"
            >
              {tgTesting ? (
                <Loader2 className="w-4 h-4 animate-spin" />
              ) : (
          ...
          <details className="group">
            ...
            <div className="mt-3 p-4 rounded-xl bg-muted/50 border border-border space-y-2 text-xs text-muted-foreground">
              ...
              <p className="pt-1">
                <strong className="text-foreground">2. TELEGRAM_CHAT_ID</strong>
              </p>
              ...
              <p>
                • Найти поле{" "}
                <code className="bg-muted px-1 py-0.5 rounded">chat.id</code> —
                это и есть CHAT_ID (для групп отрицательное число)
              </p>
              <p className="pt-1">
                <strong className="text-foreground">
                  3. Добавить в Vercel
              ...
              <p>
                • Dashboard → Settings → Environment Variables → добавить обе
                переменные → Redeploy
              </p>
            </div>
          </details>
