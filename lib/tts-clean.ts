...
function expandUnits(s: string): string {
  // КРИТИЧНО: ElevenLabs v2 на русском "заикается" если слово в конце фразы без точки/запятой.
  // Поэтому все валютные/единичные расшифровки идут с пунктуационным якорем.
  // Без этого было: "200 ₽" → "200 руб" → озвучивает как "двести рублей лей"
  ...
  // Валюты одиночные — с запятой/пробелом до и после, чтобы модель закрыла слово
  s = s.replace(/(\d)\s*₽/g, "$1 рублей");
  s = s.replace(/₽/g, " рублей");
  s = s.replace(/(\d)\s*\$/g, "$1 долларов");
  s = s.replace(/\$/g, " долларов");
  s = s.replace(/(\d)\s*€/g, "$1 евро");
  s = s.replace(/€/g, " евро");
  s = s.replace(/£/g, " фунтов");
  s = s.replace(/¥/g, " йен");

  // Метрические единицы — с пробелом-разделителем для модели.
  // Сначала составные (м/с, м.п., м/мин), потом одиночные.
  s = s.replace(/(\d)\s*м\s*\/\s*с(?![а-яА-Я])/gi, "$1 метров в секунду");
  s = s.replace(/(\d)\s*м\s*\/\s*мин(?![а-яА-Я])/gi, "$1 метров в минуту");
  s = s.replace(/(\d)\s*м\s*\/\s*ч(?![а-яА-Я])/gi, "$1 километров в час");
  s = s.replace(/(\d)\s*п\.?\s*м\.?(?![а-яА-Я])/g, "$1 погонных метров");
  s = s.replace(/п\.?\s*м\.?(?![а-яА-Я])/g, " погонных метров");

  // Кубо/квадратные метры — несколько вариантов написания (м3, м³, куб.м, кв.м)
  s = s.replace(/(\d)\s*куб\.?\s*м\.?(?![а-яА-Я])/gi, "$1 кубометров");
  s = s.replace(/куб\.?\s*м\.?(?![а-яА-Я])/gi, " кубометров");
  s = s.replace(/(\d)\s*кв\.?\s*м\.?(?![а-яА-Я])/gi, "$1 квадратных метров");
  s = s.replace(/кв\.?\s*м\.?(?![а-яА-Я])/gi, " квадратных метров");
  s = s.replace(/(\d)\s*м[³3](?![а-яА-Я])/gi, "$1 кубометров");
  s = s.replace(/м[³3](?![а-яА-Я])/gi, " кубометров");
  s = s.replace(/(\d)\s*м[²2](?![а-яА-Я])/gi, "$1 квадратных метров");
  s = s.replace(/м[²2](?![а-яА-Я])/gi, " квадратных метров");

  // Прочие единицы
  s = s.replace(/(\d)\s*мм(?![а-яА-Я])/gi, "$1 миллиметров");
  s = s.replace(/(\d)\s*см(?![а-яА-Я])/gi, "$1 сантиметров");
  s = s.replace(/(\d)\s*км(?![а-яА-Я])/gi, "$1 километров");
  s = s.replace(/(\d)\s*кг(?![а-яА-Я])/gi, "$1 килограмм");
  s = s.replace(/(\d)\s*гр?(?![а-яА-Я])/gi, "$1 граммов");
  s = s.replace(/(\d)\s*т(?![а-яА-Я])/g, "$1 тонн");
  s = s.replace(/(\d)\s*л\.?(?![а-яА-Я.])/gi, "$1 литров");
  s = s.replace(/(\d)\s*шт\.?(?![а-яА-Я])/gi, "$1 штук");

  // Одиночное "м" в конце слова или после числа (если не "мм", "см", "м³", "м²", "м/с" — все обработаны выше)
  s = s.replace(/(\d)\s+м(?![а-яА-Я])/g, "$1 метров");
...
function normalizePunctuation(s: string): string {
  ...
  // Многоточие → "..." (ElevenLabs делает естественную паузу-размышление)
  s = s.replace(/\.{3,}/g, "...");
  s = s.replace(/!{2,}/g, "!");
  s = s.replace(/\?{2,}/g, "?");

  // Лишние запятые (после расшифровок типа ", рублей за кубометр,")
  s = s.replace(/,\s*,+/g, ",");
  s = s.replace(/\s*,\s*([.!?:])/g, "$1"); // ", ." → "."
  s = s.replace(/^[\s,]+/, "");
  s = s.replace(/[\s,]+$/, "");
  ...
  // ВАЖНО: гарантируем точку в конце фразы (для естественной концовки)
