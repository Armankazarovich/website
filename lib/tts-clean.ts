...
function expandUnits(s: string): string {
  ...
  s = s.replace(/₽\s*\/\s*м[³3]/gi, " руб за кубометр");
  s = s.replace(/₽\s*\/\s*шт\.?/gi, " руб за штуку");
  s = s.replace(/₽\s*\/\s*м[²2]/gi, " руб за квадратный метр");
  s = s.replace(/₽\s*\/\s*п\.?\s*м\.?/gi, " руб за погонный метр");
  s = s.replace(/₽\s*\/\s*уп\.?/gi, " руб за упаковку");
  s = s.replace(/₽\s*\/\s*кг/gi, " руб за килограмм");
  s = s.replace(/₽\s*\/\s*т(?![а-яА-Я])/gi, " руб за тонну");
