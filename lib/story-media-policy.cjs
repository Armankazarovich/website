...
// Решение Армана 10.09.2026: сторис 1.0 принимает ролик до трёх минут.
// Длиннее — в версии 1.1 сайт сам нарежет его на части по 59 секунд.
const STORY_MEDIA_MAX_DURATION_SECONDS = 180;
const STORY_MEDIA_DURATION_TOLERANCE_SECONDS = 1;
...
// Длину исходника узнаёт FFmpeg перед сжатием. Если длина не определилась
// (так бывает у WebM из записи экрана), ролик не отвергается: итоговую копию
// всё равно проверит validateStoryMediaProbe.
function validateStorySourceDuration(seconds) {
  const value = Number(seconds);
  if (!Number.isFinite(value) || value <= 0) return { ok: true, unknown: true };
  if (value > STORY_MEDIA_MAX_DURATION_SECONDS + STORY_MEDIA_DURATION_TOLERANCE_SECONDS) {
    return {
      ok: false,
      reason: `Видео длиннее 3 минут (${Math.floor(Math.round(value) / 60)}:${String(Math.round(value) % 60).padStart(2, "0")}). Обрежьте его или разделите на две сторис. Ваш файл цел.`,
    };
  }
  return { ok: true };
}
...
module.exports = {
  STORY_MEDIA_MAX_DURATION_SECONDS,
