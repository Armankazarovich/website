...
function sourceDurationSeconds(ffmpegPath, sourcePath) {
  // "-t 0 -f null -": FFmpeg только читает оглавление исходника и сразу выходит.
  const inspection = runFfmpeg(ffmpegPath, ["-hide_banner", "-i", sourcePath, "-t", "0", "-f", "null", "-"], 60_000);
  return durationSeconds(inspection.stderr);
}

// Правило «до трёх минут» проверяется до сжатия: длинный ролик не занимает
// очередь зря, а менеджер видит понятную причину, а не общую ошибку.
function assertStorySourceDuration(ffmpegPath, sourcePath) {
  const verdict = validateStorySourceDuration(sourceDurationSeconds(ffmpegPath, sourcePath));
  if (verdict.ok) return;
  const error = new Error(verdict.reason);
  error.publicMessage = verdict.reason;
  throw error;
}
...
function createPoster(job, sourcePath, posterProcessingPath, timeout = FFMPEG_TIMEOUT_MS) {
...
// Обложка из кадра для видео, которое не нужно облегчать, и для старых сторис без обложки —
// замечание Армана 11.09.2026: вместо белой карточки со значком покупатель видит кадр из ролика.
// Кадр пишется во временный файл и переименовывается: полуготовая обложка не видна никогда.
function createStoryPosterFile({ ffmpegPath, sourcePath, posterPath, timeout = 60_000 }) {
  if (!ffmpegPath || !fs.existsSync(ffmpegPath)) throw new Error("Bundled FFmpeg is unavailable");
  const posterProcessingPath = processingPath(posterPath, ".jpg");
  fs.rmSync(posterProcessingPath, { force: true });
  try {
    createPoster({ ffmpegPath }, sourcePath, posterProcessingPath, timeout);
    fs.renameSync(posterProcessingPath, posterPath);
  } finally {
    fs.rmSync(posterProcessingPath, { force: true });
  }
  return posterPath;
...
function runStoryMediaJob(jobPath, options = {}) {
  ...
  try {
    ...
    if (!media) {
      assertStorySourceDuration(job.ffmpegPath, job.sourcePath);
    ...
    job = updateJob(jobPath, job, {
      ...
      error: error?.publicMessage || safePublicError(),
