...
function completeStoryVideoUpload(plan, options = {}) {
  if (!plan?.optimize) {
    // Короткое видео не облегчаем, но обложку из кадра делаем (замечание Армана 11.09.2026):
    // без неё покупатель видит пустую карточку. Файл видео не меняется ни на байт; если кадр
    // снять не вышло, видео всё равно сохраняется — обложка не обязательна.
    let posterUrl = null;
    try {
      const { createStoryPosterFile } = require("./story-media-worker.cjs");
      const posterName = `${plan.outputStem}-poster.jpg`;
      createStoryPosterFile({
        ffmpegPath: options.ffmpegPath || require("ffmpeg-static"),
        sourcePath: plan.sourcePath,
        posterPath: path.join(path.dirname(plan.sourcePath), posterName),
      });
      posterUrl = `${plan.sourceUrl.slice(0, plan.sourceUrl.lastIndexOf("/") + 1)}${posterName}`;
    } catch {
      posterUrl = null;
    }
    return {
