...
/**
 * ArayOrb — живой золотой шар Арая ☀️
 * Вращающиеся орбиты, дышащее свечение, мерцающий ободок.
 * Лёгкий — все анимации через SVG animate (GPU-ускоренные).
 */
  ...
  return (
    <div className="relative inline-flex items-center justify-center" style={{ width: size, height: size }}>
      <svg width={size} height={size} viewBox="0 0 100 100" style={{ display: "block", overflow: "visible" }}>
        <defs>
          ...
          <radialGradient id={`${id}-rim`} cx="50%" cy="50%" r="50%">
            ...
            <stop offset="90%"  stopColor="#ff9500" stopOpacity="0.45">
              <animate attributeName="stop-opacity" values="0.45;0.65;0.45" dur="2.5s" repeatCount="indefinite"/>
            </stop>
            <stop offset="100%" stopColor="#ffcc00" stopOpacity="0.70">
              <animate attributeName="stop-opacity" values="0.70;0.90;0.70" dur="2.5s" repeatCount="indefinite"/>
            </stop>
          </radialGradient>
        ...
        {/* Базовый шар */}
        <circle cx="50" cy="50" r={r} fill={`url(#${id}-base)`}/>
        <circle cx="50" cy="50" r={r} fill={`url(#${id}-dark)`}/>
        <circle cx="50" cy="50" r={r} fill={`url(#${id}-rim)`}/>

        {/* ── Вращающиеся орбиты — ЖИВОЙ ШАР! ── */}
        ...
        {/* Ободок — мерцающий */}
        <circle cx="50" cy="50" r={r} fill="none" stroke="rgba(255,160,30,0.35)" strokeWidth="1.5">
