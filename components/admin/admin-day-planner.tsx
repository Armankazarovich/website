...
function TaskRow({
  ...
  return (
    <div className={`flex items-center gap-2.5 px-3 py-2.5 rounded-xl transition-all group
      ${task.done ? "opacity-50" : classic ? "hover:bg-muted/50" : "hover:bg-card/[0.05]"}`}>
      {/* Приоритет */}
      ...
      <button
        onClick={onToggle}
        className={`w-5 h-5 rounded-xl border flex items-center justify-center shrink-0 transition-all
          ${task.done
            ? "bg-primary border-primary"
            : classic ? "border-border hover:border-primary/60" : "border-border/20 hover:border-primary/60"
          }`}
      >
        {task.done && <Check className={`w-3 h-3 ${classic ? "text-primary-foreground" : "text-foreground"}`} />}
      </button>

      {/* Текст */}
      <span className={`flex-1 text-sm leading-snug min-w-0 truncate
        ${task.done ? classic ? "line-through text-muted-foreground/50" : "line-through text-foreground/35" : classic ? "text-foreground" : "text-foreground/80"}`}>
        {task.text}
      </span>

      {/* Время */}
      {task.time && (
        <span className={`text-[10px] font-mono shrink-0 ${classic ? "text-muted-foreground/40" : "text-foreground/35"}`}>{task.time}</span>
      )}
      ...
      <button
        ...
        className={`w-6 h-6 rounded-xl flex items-center justify-center transition-all shrink-0 aray-icon-spin
          ${task.reminder ? "text-primary bg-primary/15" : classic ? "text-muted-foreground/30 opacity-0 group-hover:opacity-100" : "text-foreground/25 opacity-0 group-hover:opacity-100"}`}
      >
      ...
      <button
        onClick={onDelete}
        className={`w-6 h-6 rounded-xl flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all shrink-0 aray-icon-spin ${classic ? "text-muted-foreground/30 hover:text-destructive" : "text-foreground/20 hover:text-red-400"}`}
      >
  ...
  return (
    <>
      ...
      >
        ...
        {total > 0 && done < total && (
          <span className="absolute -top-0.5 -right-0.5 w-3.5 h-3.5 rounded-full text-[8px] font-bold text-foreground flex items-center justify-center"
      ...
      {open && (
        <PopupPortal>
          ...
          {/* Backdrop */}
          <div
            className="fixed inset-0 z-[70] bg-background/50 "
          ...
          >
            ...
            <div className="px-5 py-4 flex items-center justify-between shrink-0"
              ...
              <div>
                <p className={`text-[10px] font-bold uppercase tracking-[0.18em] ${classic ? "text-muted-foreground/60" : "text-foreground/30"}`}>Мой день</p>
                <p className={`text-sm font-medium capitalize mt-0.5 ${classic ? "text-foreground" : "text-foreground/80"}`}>{todayLabel}</p>
              </div>
              <button onClick={() => setOpen(false)}
                className={`w-8 h-8 rounded-xl flex items-center justify-center hover:bg-card/10 transition-colors`}
            ...
            {total > 0 && (
              <div className="px-5 py-3 shrink-0" style={classic ? {
                ...
                <div className={`flex items-center justify-between text-[11px] mb-1.5 ${classic ? "text-muted-foreground/60" : "text-foreground/40"}`}>
                  <span>Выполнено {done} из {total}</span>
                  <span className="text-primary font-semibold">{pct}%</span>
                </div>
                <div className={`h-1.5 rounded-full overflow-hidden ${classic ? "bg-muted" : "bg-card/10"}`}>
                  <div
            ...
            <div className="flex-1 overflow-y-auto py-2">
              {total === 0 && (
                <p className={`text-center text-sm py-10 ${classic ? "text-muted-foreground/40" : "text-foreground/25"}`}>
                ...
                return (
                  <div key={sec.label} className="mb-1">
                    <button
                      ...
                      className={`w-full flex items-center gap-2 px-4 py-1.5 text-left transition-colors ${classic ? "hover:bg-muted/50" : "hover:bg-card/[0.04]"}`}
                    >
                      <sec.icon className={`w-3 h-3 ${classic ? "text-muted-foreground/50" : "text-foreground/30"}`} />
                      <span className={`text-[10px] font-bold uppercase tracking-[0.16em] flex-1 ${classic ? "text-muted-foreground/60" : "text-foreground/30"}`}>
                        {sec.label}
                      </span>
                      <span className={`text-[10px] ${classic ? "text-muted-foreground/40" : "text-foreground/20"}`}>{secTasks.length}</span>
                      {isCollapsed ? <ChevronDown className={`w-3 h-3 ${classic ? "text-muted-foreground/40" : "text-foreground/20"}`} /> : <ChevronUp className={`w-3 h-3 ${classic ? "text-muted-foreground/40" : "text-foreground/20"}`} />}
                    </button>
              ...
              {noTimeTasks.length > 0 && (
                <div className="mb-1">
                  <div className="flex items-center gap-2 px-4 py-1.5">
                    <Clock className={`w-3 h-3 ${classic ? "text-muted-foreground/50" : "text-foreground/30"}`} />
                    <span className={`text-[10px] font-bold uppercase tracking-[0.16em] ${classic ? "text-muted-foreground/60" : "text-foreground/30"}`}>Без времени</span>
                  </div>
                  {noTimeTasks.map(t => (
                    <TaskRow key={t.id} task={t}
                      onToggle={() => toggleDone(t.id)}
            ...
            <div className="shrink-0 p-4 space-y-2.5"
              ...
              <input
                ...
                className={`w-full px-3.5 py-2.5 rounded-xl text-sm outline-none ${classic ? "bg-muted text-foreground placeholder:text-muted-foreground/50 border border-border" : "text-foreground placeholder:text-foreground/25"}`}
                style={classic ? {} : {
              ...
              <div className="flex items-center gap-2">
                <input
                  ...
                  className={`flex-1 px-3 py-2 rounded-xl text-sm outline-none ${classic ? "bg-muted text-foreground border border-border" : "text-foreground/70"}`}
                  style={classic ? {
                    colorScheme: "light",
                  } : {
                    background: "rgba(255,255,255,0.07)",
                ...
                <button
                  ...
                  className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 transition-all
                    ${reminder ? "bg-primary/20 text-primary" : classic ? "text-muted-foreground/40 hover:text-muted-foreground/70" : "text-foreground/30 hover:text-foreground/60"}`}
                  style={classic ? {
