...
const GLOBAL_HINTS_BY_HREF: Record<string, string> = {
  ...
  "/admin/suppliers": "Служебные поставщики",
...
const SEARCH_CONTEXTS: SearchContext[] = [
  ...
  },
  {
    match: "/admin",
    label: "Рабочий стол",
    placeholder: "Найти товар, заказ, клиента, рекламу или раздел...",
    nextStep: "Проверить рабочий стол по реальным сигналам",
    quick: DEFAULT_ADMIN_QUICK,
    hints: ["товары", "каталог", "заказ", "реклама"],
  },
  {
    match: "/cabinet",
    label: "Кабинет",
...
export function useAdminSmartSearch({
  ...
  const queryHints = useMemo(() => {
    const fallback = role === "USER" ? ["мои заказы", "каталог", "профиль"] : ["товары", "каталог", "заказ", "реклама"];
