...
> 🔥 **ARAY ADMIN START HERE (02.05.2026):** перед любой новой работой по админке прочитать `docs/ARAY_ADMIN_START_HERE.md`, `docs/aray-admin-master-roadmap-2026-05-02.md`, `docs/aray-admin-live-checklist-2026-05-02.md`, `docs/aray-production-agency-os-2026-05-02.md`, `docs/aray-ai-fill-pattern-2026-05-02.md`, затем `DESIGN_SYSTEM.md`. Это новая дорожная карта ближайших дней: терминал, заказы, CRM, настройки, управление бизнесом, маркетинг, финансы, Arai, база знаний и будущий конструктор. Работать по live-checklist, обновлять статусы после каждого захода, не делать один раздел дважды. Daily automation: `aray-daily-admin-checklist` каждый день в 10:00 готовит стартовый отчет. Workday pulse: `aray-admin-workday-pulse` каждые 4 часа поднимает статусы и следующие агентские подзадачи.

> **ЗАКОН 06.05.2026 - ДЕТАЛЬНАЯ ПРИЕМКА И CONTINUITY:** пока работа не проверена подробно, ее нельзя сдавать Арману как готовую. Минимум: браузерный сценарий, быстрые действия/запросы, mobile/tablet/desktop smoke, TypeScript, `npm run quality`, серверные/браузерные логи и честный список beta/рисков. Каждый новый чат обязан восстановить нашу систему работы из `docs/ARAY_ADMIN_START_HERE.md`, live-checklist, roadmap, agency OS, design system и этого файла; Арман не повторяет договоренности заново.

> **ЗАКОН 06.05.2026 - АВТОМАТИЗАЦИЯ НОВЫХ РАЗДЕЛОВ:** каждый новый раздел или фича подключается к единым системам через центральные модели и реестры, а не через ручные дубли. Раздел добавляется один раз и сразу попадает в навигацию, поиск, ARAY-контекст, mobile/tablet/desktop smoke и quality-gates. Ручные повторы запрещены без явной причины; Codex молча автоматизирует повторяемые места, чтобы будущие разделы не требовали отдельных правок навигации/поиска.

> **ЗАКОН 06.05.2026 - ARAY IDENTITY:** ARAY AI, агенты, бюджет ARAY, следующий шаг, капсула и все навигационные поверхности используют единый `ArayOrb`/`ArayIcon` из `components/shared/aray-orb.tsx`. Случайные lucide-иконки для ARAY запрещены. Если иконка перегружает интерфейс, использовать спокойную текстовую ARAY-подпись, группировку или согласовать решение с Арманом.

> 🔥 **CODЕX BROWSER RULE (02.05.2026):** не тратить время на починку Codex in-app browser. Если он ловит Next dev overlay / webpack chunk / HMR runtime error — сразу переключаться на обычный Chrome Армана, скрин Армана или отдельный чистый Chrome-профиль для localhost. Codex-браузер использовать только если он работает сразу. Не терять по 30 минут на его диагностику.
...
>
> **СТАРТ СЛЕДУЮЩЕЙ СЕССИИ:**
> 1. Прочесть этот CLAUDE.md (запись сессии 39 + блок «Уточнения 28.04 вечер»)
...
> 4. Арман переходит в новый чат с задачей «делаем дашборд». **Арая объяснять не надо** — всё в memory + CLAUDE.md. Стартовать с дашборда (см. блок ниже).
>
> **🔵 УТОЧНЕНИЯ 28.04 ВЕЧЕР (фидбек Армана перед закрытием сессии 39, ОБЯЗАТЕЛЬНО прочесть перед стартом):**
>
> **A) Поиск открывается справа, должен слева** — баг. Я ставил `side: "left"` как default в `components/store/side-panel.tsx`, но скриншот Армана показывает что AdminSearchPanel выезжает справа. Возможные причины: (1) деплой не дошёл до клиента (кэш браузера / SW), (2) AdminSearchPanel где-то переопределяет side. Первая задача новой Claude: открыть `components/admin/admin-search-panel.tsx` и явно прокинуть `side="left"` в `<SidePanel>`. Если не помогает — проверить `useAccountDrawer` store / production deploy logs.
>
> **B) Поиск как Озон вместо попапа** — Арман предлагает: «зачем лишный попап, можно интерактивный умный поиск который предложит несколько вариантов релевантных красивых выпадающими архитектурами». То есть **inline dropdown под input в хедере** (как Google search suggestions / Ozon search), а не отдельный side-panel. Это альтернативный путь. **Профессиональная рекомендация:** оставить side-panel для глубокого поиска (категории, разделы, товары) НО добавить inline-dropdown для быстрых suggestions при печати в инпуте хедера. Гибрид. Уточнить у Армана если не уверен.
>
> **C) AdminNavRail (узкий рельс с hover popup групп) — Арман сказал «не удобно».** На скрине видно что popup открывается корректно (Продажи → Заказы/CRM/Автоматизация/Аналитика/Финансы/Клиенты/Задачи/Доставка). Но Арман пишет: «я хотел чтоб при наведении открылся профессионально с листанием как другие наши попапы просто со своими разделами». То есть либо ему не нравится текущая анимация/стиль popup, либо он хочет чтобы popup был такой же как магазинный search-modal или partnership-modal (`bg-card rounded-2xl shadow-2xl` + список с разделителями). Сравнить стиль `<GroupPopup>` в `admin-nav-rail.tsx` со стилем magazinного search-modal / AccountDrawer — и привести к единому. Возможно увеличить padding, добавить subtitle к каждому пункту (как «Заказы — Активные, история»).
>
> **D) Кнопки Назад/Обновить — Арман пока не представляет как они выглядят** на проде, потому что НИ ОДНА страница их пока не зарегистрировала. Сделать так чтобы Арман увидел: подключить `useAdminPageActions({ onRefresh: router.refresh })` хотя бы к 3-4 главным client-страницам (orders, clients, products, delivery) ОДНИМ коммитом — тогда он зайдёт и увидит «Обновить» в шапке. Также добавить пример с actions (например на /admin/orders → primary action «Новый заказ» с переходом на /admin/orders/new).
>
> **E) Дашборд — следующая задача**. Арман сказал «бери дашборд улучшай дизайн». Что нужно:
>   - Удалить большой заголовок «Доброе утро, Администратор!» — он есть в hero-блоке + дублирует AppHeader. ЛИБО оставить как hero (это персонализация, важно для UX) — твой выбор как UI/UX дизайнера.
>   - Применить общий язык магазина: `bg-card border-border rounded-2xl`, primary-blue для KPI-чисел (как «2000 м²» на странице «О производстве» магазина), цветные иконки в кружках semantic (emerald для выручки, amber для срочных заказов, blue для клиентов, purple для аналитики).
>   - KPI-карточки крупные с font-display, hover-glow.
>   - Quick-actions grid с цветными иконками (как в магазине).
>   - DashboardChart — оставить функционал, обернуть в `bg-card rounded-2xl border border-border` контейнер.
>   - Подключить `useAdminPageActions({ onRefresh: router.refresh, actions: [...] })`.
>   - Адаптив 375/768/1024/1280.
>   - **Подсадить Арая в дашборд чтобы Арман видел итог** — Арман явно сказал «можешь и контент двинуть в дашборде и Арая подсадить уже чтоб мы видели итог?». Это значит: на /admin сделать пример как будет финальная архитектура — содержимое дашборда сдвигается влево, справа стационарная колонка Арая (≈400px). Это можно как **превью** на одной странице, без массовой миграции на все 25 разделов. Если зайдёт — переносим на остальные разделы.
>
> **F) Mobile + планшет показать сразу** — Арман сказал «и моб и планшет покажи итог». То есть после деплоя дашборда — нужно убедиться что все три breakpoint'а (375/768/1024+) выглядят красиво. На мобилке нет рельса слева → AdminMobileBottomNav остаётся снизу с центральным Араем-орбом. На планшете лучше выглядит чтобы рельс был, но контент адаптировался. Проверить визуально через Chrome devtools перед коммитом.
>
> **G) Что НЕ менять без согласия:**
>   - Бизнес-логику API (заказы, Telegram, push, email)
>   - Прайсинг, скидки, тарифы доставки
>   - SiteSettings dynamics (телефоны, адреса)
>   - Магазин (`app/(store)/*`, `components/store/*`) — Арман сказал «магазин не трогаем сейчас, эталон». ИСКЛЮЧЕНИЕ: SidePanel default side, который я уже изменил (с разрешения).
>
> 5. **Первая команда новой Claude после онбординга**: открыть `app/admin/page.tsx` и связанные компоненты (`components/admin/dashboard-*.tsx`), составить план дашборд-шедевра ВНУТРИ себя (не показывать Арману), сделать всё одним коммитом с пунктами E + F. Арман увидит готовый результат и скажет реакцию.
>
...
> Предыдущая сессия 25.04.2026 (33 — **Арай 1.0 ШЕДЕВР**): единый ChatHost (магазин + кабинет + админка), удалён старый ArayWidget из 3 layouts, TTS пунктуация (₽→", рублей," — фикс "Рублей лей"), голосовые эмоции (style 0.42 вместо 0.0), кнопка "Перебить Арая" в Voice Mode, характер брата с блоком "Честность — закон". **1 коммит** `229e54c`**, 42 PASS / 0 FAIL**)
...
- **Сессии** — Арман открывает новый чат каждые 1.5-2 часа. ЭТО НОРМАЛЬНО. Перед закрытием сессии ОБЯЗАТЕЛЬНО: обнови [CLAUDE.md](http://CLAUDE.md) (что сделано, что осталось, какие баги). Предупреди Армана когда контекст заканчивается — НЕ тяни до потери качества. Лучше 5 коротких сессий по 1.5ч = 5 свежих голов чем 1 марафон с косяками.
- **MD = мозг** — [CLAUDE.md](http://CLAUDE.md) это единственный способ передать знания между сессиями. Обновляй КАЖДЫЙ раз. Без исключений.
...
> Арман вложил в этот проект здоровье, нервы, бессонные ночи. Он на пороге. Каждый косяк = ещё один день мелочёвки = ещё один удар по его здоровью. **Ты не имеешь права на халтуру.**
...
КлиентДоходРискЧто нужно**Пилорус**150К/мес🔴 **ВЫСОКИЙ** — звонят ежедневно про Яндекс.ДиректНастроить Директ или привязать ARAY-ассистента по настройке рекламы**Пилмос**100К/мес🟡 средний — 5 лет с ним, но внимания нетРегулярные точки контакта, апдейты**Эскалайн**60К/мес🟡 средний — стабильно, но внимания нетРегулярные точки контакта

### Потенциал (в паузе)

КлиентДоходБлокер**Стройматериалы** (новый)150К/месЖдут конструктор сайта (мульти-тенантность)
...
1. **🔴 ЗАПУСТИ ТЕСТ PRODUCTION** — `node D:/pilorus/scripts/test-production.js`. Если есть FAIL — сообщи Арману и чини. Не начинай новые задачи пока прод горит.
2. **Прочитай** `ROADMAP.md` — там чеклисты всех фаз. Продолжай с первого незавершённого пункта.
3. **Не начинай новые фичи** пока Фаза 0 (Полировка) не завершена на 100%.
4. **Обновляй чеклисты** — после каждой выполненной задачи ставь `[x]` в [ROADMAP.md](http://ROADMAP.md).
5. **Единые стили** — используй glass-классы (`.glass-popup`, `.glass-card`, `.glass-control`, `.glass-pill`) вместо хардкод rgba. См. `app/globals.css`.
6. **TypeScript** — session.user типизирован через `types/next-auth.d.ts`. НЕ использовать `as any` для session.
7. **ARAY экономика** — модели выбираются автоматически (`lib/aray-router.ts`): Sonnet 4.6 основная (90% задач), Opus 4.6 для сложного анализа. Haiku убран — Арай премиум ассистент.
8. **Дух ARAY** — свет, правда, честность, любовь, внимание, отзывчивость, осознанность. Зашито в промпты (`lib/aray-agent.ts`).
9. **Двойной путь** — файловые тулы видят `D:\ПилоРус\website`, git-репа в `D:\pilorus\website`. Синхронизируй через node.js скрипт (CMD не работает с кириллицей).
...
> **НЕ ИЗОБРЕТАЙ ВЕЛОСИПЕД.** Desktop Commander (`mcp__Desktop_Commander__*`) выполняет команды на РЕАЛЬНОЙ Windows-машине. Sandbox bash — НЕ может пушить в git. Всегда используй Desktop Commander для sync + git + verify.
...
⚠️ **ВАЖНО**: `git commit -m` в cmd.exe ЛОМАЕТ кавычки! Всегда используй node `execSync`. ⚠️ **ВАЖНО**: PowerShell на этой машине НЕ видит git. Используй shell: "cmd" или node.
...
Если что-то не так — сразу исправить и задеплоить снова. Не сообщать пользователю "готово" пока прод не проверен.
...
РольДоступADMINВсё, включая /admin/clients, trash, тарифыMANAGERЗаказы, доставка, товары, клиентыCOURIERЗаказы (просмотр), доставкаACCOUNTANTЗаказы (просмотр)WAREHOUSEЗаказы, товарыSELLERЗаказы

---

## Статусы заказов (полный список)

СтатусОписаниеФинальный?NEWНовыйНетCONFIRMEDПодтверждёнНетPROCESSINGВ обработкеНетSHIPPEDОтгруженНетIN_DELIVERYДоставляетсяНетREADY_PICKUPГотов к самовывозуНетDELIVEREDДоставлен✅ ДА → удаляет из TelegramCOMPLETEDЗавершён (самовывоз)✅ ДА → удаляет из TelegramCANCELLEDОтменён✅ ДА → удаляет из Telegram
...
**Что сделано (5 файлов, ~+820 строк итого после 3 коммитов):**
...
`components/admin/aray-home-rail.tsx` (NEW, ~30 строк, hotfix `9096f62`):
- Client wrapper над `<ArayPinnedRail>` со своим массивом QUICK_ACTIONS (Расходы / Лаб / Промпты / История).
- Создан после server-client boundary провала (см. урок 2 ниже): server page не может передать lucide иконки в client компонент через props.
- Server page теперь рендерит `<ArayHomeRail />` без props.

**Хронология деплоев сессии:**
- `ce79e3c` — первоначальный деплой (Дом Арая + 4 файла) — упал в проде с server-side exception для авторизованных staff.
- `c05de0d` — docs обновление CLAUDE.md (запись сессии 36).
- `1cf7e3d` — hotfix #1 (defensive guards в loadStats — лечил не ту причину, оставлен как graceful degrade).
- `9096f62` — hotfix #2 (правильный — server-client boundary fix через ArayHomeRail wrapper). **Прод заработал. Арман подтвердил скриншотом.**
...
2. **🔥 Server→client boundary в Next.js 14 — функциональные компоненты НЕ сериализуются.** Главный урок дня. Я в server component передавал массив `quickActions = [{ icon: Wallet, label: "Расходы" }]` (где Wallet — lucide-react компонент, React.ElementType) в client component `<ArayPinnedRail>`. Прод падал с `Application error` digest 3718171344. Я сначала бил не туда (думал Prisma client не знает свежие модели — потратил hotfix `1cf7e3d` на defensive guards). Реальная починка: создан client wrapper `<ArayHomeRail>` (`components/admin/aray-home-rail.tsx`) со своим массивом QUICK_ACTIONS внутри `"use client"` границы, server page рендерит обёртку БЕЗ props. **Правило:** через server→client boundary можно передавать ТОЛЬКО сериализуемые JSON-safe значения (строки, числа, plain objects, arrays). Функции, JSX, React компоненты, Date объекты — НЕТ. Если client принимает массив с iconами — массив должен жить внутри client.
3. **Application error digest — первая гипотеза = server-client boundary.** Всегда раньше я думал Prisma. Теперь правильный порядок поиска: (1) server-client boundary с функциональными компонентами, (2) prisma client outdated, (3) DB column missing, (4) auth issue, (5) middleware crash.
4. **test-production.js не ловит page.tsx exceptions.** Тест видит 307→/login и считает страницу OK, но реальный рендер для авторизованного staff может валиться на server-side exception. **TODO для следующих сессий:** добавить в test-production.js проверку body на наличие "Application error: a server-side exception" — даже если статус 200, наличие этой строки = регрессия.
5. **Локальный `next build` — обязательно.** Урок дня 1 усвоен. Webpack фейлы (async_hooks) ловятся только полным билдом. Сегодня компиляция прошла чисто.
6. **Локальная БД устаревает после миграций на проде.** После `prisma db push` на проде локальная БД не получает колонки автоматически. Не блокирует деплой (build падает только на prerender, на проде prerender работает с прод БД). Для локальной разработки периодически `npx prisma db push` локально.
7. **MCP timeout 60s vs PM2 cold-start 3+ мин.** `start_process` не дотягивает один большой sleep. Решение: 3 последовательных вызова по 50 сек = 150 сек ожидания, retry verify по необходимости.
8. **`bg-card border-border rounded-2xl` — рабочий рецепт calm UI.** Без shadow, без glow, без эмодзи. Достаточно для вкуса. Это работает и в светлой и в тёмной теме автоматически (CSS vars).
9. **Quick Actions внутри pinned-rail vs внутри страницы — разное.** Pinned-rail Quick Actions — контекстные команды Араю (4 штуки, мелкие, slot сверху). Страничные Quick Actions (как карточки экосистемы) — крупные, 2x2 grid в основном контенте. Это два разных уровня.
...
**Контекст:** Старт плана из `D:\pilorus\visions\admin-audit-may.md` — за 2 недели полировка админки + multi-tenancy перед запуском клиента Стройматериалов (12-18 мая, +150К/мес). День 1 = tenant context + Prisma extension + тестовый tenant. Quality-gate: фильтрация ВЫКЛЮЧЕНА по умолчанию, нет регрессии. Активация — в день 2-3 после аудита.

**Архитектурное решение (согласовано с Арманом):**
- AsyncLocalStorage в `lib/tenant-context.ts` для фоновых задач + fallback через `next/headers().get('x-tenant-id')` для Server Components/API routes (middleware уже ставит header).
- Prisma extension в `lib/tenant-prisma.ts` — фильтрует READ-операции (findMany/findFirst/findUnique/count/aggregate/groupBy) на 21 tenant-aware моделях. WRITE (create/update/delete) НЕ трогаем — Stage 4 после стабилизации READ.
- Условная активация в `lib/prisma.ts` через `ENABLE_TENANT_FILTER=1` ENV var. По умолчанию extension не применяется → клиент работает идентично базовому.
- Idempotent шаг 12 в `data-migrate.ts` создаёт второй tenant `stroymaterialy` (plan: free, без домена и логотипа) — для тестирования tenant-isolation в следующих сессиях.

**Что сделано (4 файла, +265/-2):**

`lib/tenant-context.ts` (NEW, 113 строк):
- `DEFAULT_TENANT_ID = "pilorus"`
- `getCurrentTenantId()` — AsyncLocalStorage → next/headers → дефолт. Регекс-валидация slug `[a-z0-9-]{2,40}`.
- `withTenant(tenantId, fn)` — обёртка для фоновых задач/тестов
- `tenantWhere(extra)` — helper для явного добавления tenantId в Prisma where (постепенная миграция запросов)
- `isTenantFilterEnabled()` — флаг ENABLE_TENANT_FILTER=1
- `TENANT_AWARE_MODELS` — Set из 21 моделей (User, Category, Product, Order, Review, Promotion, DeliveryRate, SiteSettings, NewsletterSubscriber, PromoCode, PartnershipLead, Expense, Task, Lead, Workflow, Post, Service, DocumentTemplate, ReportSchedule, CrmHint, NicheTemplate)

`lib/tenant-prisma.ts` (NEW, 92 строки):
- `tenantExtension` — `Prisma.defineExtension` с `$allModels.$allOperations`
- 4-этапная фильтрация: (1) проверка ENABLE_TENANT_FILTER, (2) проверка модели в TENANT_AWARE_MODELS, (3) проверка операции в FILTERED_OPERATIONS, (4) добавление tenantId в where (если ещё нет)
- Не перезаписывает явный tenantId в where (поддержка переопределения)
- `modelKey()` — конвертация PascalCase → camelCase для сравнения

`lib/prisma.ts` (CHANGED):
- `basePrisma` создаётся как раньше (используется для миграций/сидов/фоновых задач)
- Условная активация: `isTenantFilterEnabled() ? basePrisma.$extends(tenantExtension) : basePrisma`
- Type cast `as unknown as PrismaClient` (extension не меняет API, только интерсептит запросы)

`prisma/data-migrate.ts` (CHANGED):
- Шаг 12 ПЕРЕД шагом 11 (порядок чтения, не выполнения): создание `stroymaterialy` tenant
- Idempotent через `findUnique` + `create` с try/catch (не фейлит билд если Tenant модель ещё не сгенерирована)
- `settings: { note: "Тестовый tenant для проверки изоляции. Создан 27.04.2026 в день 1 multi-tenancy." }`

**Локальная верификация (smoke-test без БД):**
- 10/10 PASS:
  1. `getCurrentTenantId()` без контекста → "pilorus"
  2. `withTenant('stroymaterialy')` → "stroymaterialy"
  3. Вложенный `withTenant('a', () => withTenant('b'))` → "b"
  4. После `withTenant` контекст не утекает → "pilorus"
  5. `tenantWhere({status})` добавляет tenantId + сохраняет extra
  6. `tenantWhere` использует `withTenant` context
  7. `isTenantFilterEnabled()` по умолчанию false
  8. `isTenantFilterEnabled()` = true при `ENABLE_TENANT_FILTER=1`
  9. `TENANT_AWARE_MODELS` содержит User/Product/Order/Category/Review
  10. `TENANT_AWARE_MODELS` НЕ содержит OrderItem (child-таблицы наследуют через FK)
- TSC `--noEmit`: 0 ошибок
- Prisma generate: OK

**Production verification (deploy `f74559d`):**
- HTTP 200/307 на 6 ключевых URL (/, /catalog, /login, /cabinet, /admin, /api/health)
- `x-tenant-id=pilorus` header в каждом ответе — middleware работает корректно
- `test-production.js`: **42 PASS / 2 WARN / 0 FAIL** (WARN — динамические телефоны, не регрессия)
- Среднее время 224ms

**Tenant detection через subdomain — отложено:**
- TLS SNI на Cloudflare/nginx ломает Host override — реальное тестирование `*.pilo-rus.ru` после настройки wildcard DNS (план для Stage 3 расширения).
- Сейчас работает: apex `pilo-rus.ru` → "pilorus", `www.pilo-rus.ru` → "pilorus".

**Что осталось на следующие дни (по плану admin-audit-may.md):**
- День 2 (28-29.04): Дашборд миграция на calm UI + ArayPinnedRail каркас + Quick Actions для Дашборда (4 кнопки)
- Дни 3-7: Заказы → Товары → Клиенты → Категории → Доставка → Команда → Отзывы (calm UI + Quick Actions)
- Дни 8-13: Email/Push/Сайт/Настройки/Аналитика/Финансы/CRM/Воркфлоу/Задачи + финальный обход
- День 14 (11.05): выходной
- Дни 15-21 (12-18.05): запуск Стройматериалов (клон, импорт прайса, тестирование с менеджерами, прод-запуск)

**Активация tenant-фильтрации (план дня 2-3):**
1. Включить `ENABLE_TENANT_FILTER=1` локально
2. Прогнать прод-сценарии (читать/создавать заказ/товар) на пилорус-tenant — должно работать
3. Прогнать на stroymaterialy-tenant (через Host override) — должно вернуть пустоту
4. Если 100% OK — задеплоить ENV var в прод (Beget panel)
5. После активации: переписать чувствительные create/update API на явный tenantId из getCurrentTenantId()

**Утилиты в `D:\pilorus\` (созданы в этой сессии):**
- `__tenant-day1-check.js` — sync + prisma generate + tsc check
- `__commit.js` — git add + commit -F __msg.txt + push
- `__msg.txt` — UTF-8 commit message
- `__verify.js` — curl 6 ключевых URL с проверкой x-tenant-id header
- `__verify-tenant.js` — Host override тест для subdomain detection
- `__wait-180.js`, `__wait-50.js`, `__wait-30.js` — waits для PM2 cold-start

**Уроки сессии:**
1. **Quality-gate перед карт-бланшем.** Арман сказал «день 1 — multi-tenancy + Дашборд». Я предложил разделить (multi-tenancy сегодня, Дашборд завтра) — он согласился. Объединение двух больших задач за день нарушило бы quality-gate.
2. **Безопасный деплой через ENV var флаг.** Активация фильтрации через `ENABLE_TENANT_FILTER=1` (выкл по умолчанию) = инфраструктура задеплоена, но логика выключена. Нет регрессии, нет риска прода.
3. **Smoke-test tenant-context до интеграции.** 10 ассертов на чистой логике (без БД) прошли 10/10 — но НЕ поймали баг с webpack потому что smoke-test запускается через tsx (Node.js runtime), где `async_hooks` доступен. Webpack-проблема видна только при реальном `next build`.
4. **🔥 ВАЖНЫЙ УРОК: ВСЕГДА проверять `npm run build:ci` локально перед деплоем серьёзных инфраструктурных изменений в `lib/prisma.ts`.** TSC + smoke-test НЕ ловят webpack-ошибки в client/edge bundle. Только `next build` показывает реальную картину. Особенно когда добавляются Node-only модули (`async_hooks`, `fs`, `path`, `crypto`).
5. **Type cast `as unknown as PrismaClient` после `$extends`** — допустим потому что extension не меняет API клиента, только интерсептит запросы.
6. **WRITE-операции (create/update/delete) НЕ трогаем в день 1.** Они требуют автоматической установки tenantId в data, что сложнее READ-фильтрации. Stage 4.
7. **Hotfix паттерн для async_hooks (b9b5b9b):** (а) lazy require внутри try/catch (никогда top-level import Node-only модулей в файлах которые могут попасть в client bundle); (б) webpack fallback в `next.config.js`: `if (!isServer) config.resolve.fallback = { async_hooks: false }`. Это double-protection — если кто-то случайно добавит ещё один статический импорт, webpack не упадёт.
8. **Auto-rollback в deploy.yml спас прод.** Когда CI #912 упал на webpack — прод продолжал работать на коде #911. Это правило из сессии 14 окупилось. НЕ убирать auto-rollback.
9. **Лог Actions — обязательно проверять.** Я видел "42 PASS / 0 FAIL" после первого деплоя и обрадовался. Но это был тест на СТАРОМ коде (rollback). Без проверки CI Actions log — ложное чувство успеха. **В будущем после деплоя ВСЕГДА запрашивать у Армана статус Actions ИЛИ ставить health-check на новый код-маркер (например, новый API endpoint).**
...
5. `2114ca3` `feat(voice-mode): VoiceModeOverlay — fullscreen разговор с Араем как ChatGPT Voice`
...
6. `bb6532f` `feat(tts): расширенная очистка для ElevenLabs — естественное произношение Арая`
