...
6. **ARAY экономика** — модели выбираются автоматически (`lib/aray-router.ts`): Haiku для FAQ, Sonnet для бизнеса, Opus для анализа. Токены логируются в `ArayTokenLog`.
