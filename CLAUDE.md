...
> Последнее обновление: 27.04.2026 (сессия 35 — **Multi-tenancy day 1: tenant context + Prisma extension + тестовый tenant `stroymaterialy`**). Инфраструктурный этап перед запуском Стройматериалов (план `D:\pilorus\visions\admin-audit-may.md`, день 1). NEW `lib/tenant-context.ts` (`getCurrentTenantId/withTenant/tenantWhere/isTenantFilterEnabled/TENANT_AWARE_MODELS`), NEW `lib/tenant-prisma.ts` (Prisma extension авто-фильтрации READ-операций), CHANGED `lib/prisma.ts` (условная активация extension через `ENABLE_TENANT_FILTER=1`), CHANGED `prisma/data-migrate.ts` (шаг 12 — idempotent `stroymaterialy` tenant). Smoke-тест tenant-context 10/10 PASS, TSC 0 ошибок, **коммит** `f74559d`**, 4 файла, +265/-2 строк, 42 PASS / 0 FAIL.** По умолчанию фильтрация ВЫКЛЮЧЕНА — нет регрессии. Активация в день 2-3 после полного аудита.
...
3. **Smoke-test tenant-context до интеграции.** 10 ассертов на чистой логике (без БД) поймали бы любой баг в AsyncLocalStorage/headers fallback. Прошло 10/10 — спокойно деплоим.
4. **Type cast `as unknown as PrismaClient` после `$extends`** — допустим потому что extension не меняет API клиента, только интерсептит запросы. Альтернатива — менять тип всех импортов prisma в проекте, что огромная работа.
5. **WRITE-операции (create/update/delete) НЕ трогаем в день 1.** Они требуют автоматической установки tenantId в data, что сложнее READ-фильтрации. Stage 4.
