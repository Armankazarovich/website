...
### 🔥 ПРИОРИТЕТ 1 — Единая админка для всех ролей (БИРЖА)
Арман хочет: клиенты, продавцы, менеджеры — все в одной красивой админке.
- Роутинг по ролям: USER видит свой кабинет в стиле glass-админки
- Permissions система: каждый раздел проверяет права
- Клиенты: Мои заказы, Мои отзывы, Профиль, Уведомления, Избранное
- Продавцы: + Товары, Аналитика, Доставка
- Менеджеры: + CRM, Сотрудники, Клиенты
- Переход `/cabinet` → `/admin` с ограниченными правами (один layout, разные меню)
