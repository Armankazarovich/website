...
>
> Версия: 1.5 (04.05.2026) Источник правды: магазин (`pilo-rus.ru/`, `/catalog`, `/contacts`). Замещает: манифест ARAYGLASS (deprecated — см. раздел «Anti-patterns»).
...
## 0.1 Закон согласования

**Если паттерна нет в DESIGN_SYSTEM.md — его нельзя молча придумывать.** Дизайн-система важнее вкуса отдельной сессии. Новый визуальный ход, новый тип попапа, новая навигация, новый фон, новая карточка или новый набор токенов сначала описываются здесь и согласуются с Арманом, только потом попадают в код.

**Разрешено без согласования:** менять расстановку, порядок, группировку и текст внутри уже утверждённого паттерна. Пример: переставить разделы в меню, разбить «Продажи» на подгруппы, убрать дублирующий блок с дашборда.

**Запрещено без согласования:** менять визуальный язык паттерна. Пример: сделать новый стиль попапа вместо утверждённого drawer/account/nav-panel, добавить другой радиус, тень, blur, цветовую схему, декоративный фон или новый тип строки.

**Эталон для админских меню:** использовать уже согласованный язык аккаунт-меню и навигационного popup: `bg-card`, `border-border`, `rounded-2xl`, строки `admin-drawer-row` / `admin-nav-panel-item`, спокойная шапка, lucide-иконки, без нового визуального слоя. Если нужна новая разновидность меню — сначала добавить её в этот документ.

**Комментарий-исключение:** если редкий технический случай всё-таки требует нарушения guard-проверки, рядом с линией кода должен быть комментарий `design-system-allow` и причина. Такой комментарий не является постоянным разрешением; его нужно убрать после внесения паттерна в DESIGN_SYSTEM.md.

## 0.2 Рабочий процесс по разделам

Работаем локально и по одному разделу. Арман задаёт функцию, смысл и приоритет; Codex обязан добавить свою профессиональную точку зрения: логика раздела, расстановка, визуальная чистота, мобильная версия, ошибки и риски.

Для каждого раздела порядок такой:
1. Сначала локальная правка и локальный просмотр.
2. Сразу проверяем адаптивность: 390px mobile, 768px tablet, 1024px+ desktop. Не переносим mobile/tablet «на потом».
3. Codex даёт профессиональное мнение по разделу: что ломает UX, что не хватает, что можно улучшить без смены паттерна.
4. Арман даёт своё место: вкус, приоритет, бизнес-смысл. После этого фиксируем раздел и идём дальше.
5. Затем проверка дизайна по этому документу и `npm run quality`.
6. Продакшен, коммит и пуш — только после явной команды Армана или отдельного согласования в этой сессии.

Если в процессе найден согласованный момент, его можно сразу фиксировать в коде и документации. Если момент спорный или меняет визуальный паттерн — сначала обсудить, потом кодить.

**Закон адаптива:** всё, что делается в админке, считается незавершённым, пока не проверено на mobile/tablet/desktop. Если баг найден на одном breakpoint, ищем первопричину в общем компоненте или CSS-правиле, а не лепим отдельный костыль для одного экрана.

## 0.3 Закон единого Admin Page Frame

Все страницы админки строятся в одной резиновой системе ширины, отступов и ритма. Страница не может сама придумывать `max-w-*`, `container`, `mx-auto`, случайные root-padding или узкую колонку, если это не отдельный читаемый документ.
...
- root-контейнеры вроде `max-w-4xl`, `max-w-5xl`, `container mx-auto` на рабочих страницах админки;
- разные ширины у соседних разделов без причины;
- локальные “острова” дизайна, которые ломают общий ARAY-ритм;
- кнопки/панели, которые на mobile становятся тяжелыми, узкими или обрезают текст.

Если странице реально нужна особая ширина, рядом в коде обязателен комментарий `design-system-allow page-width` с причиной. Без причины страница приводится к единому frame.
...
**Закон читаемости:** рабочие экраны админки и терминала не проектируются на микрошрифте. Для текста, который человек должен читать во время работы, используем `text-sm` или `text-base`; `text-[10px]` и `text-[11px]` оставляем только для коротких бейджей, технических меток и вторичных счётчиков. Подписи `text-muted-foreground` должны оставаться контрастными в dark mode; не глушить важный текст через `/40`, `/50`, `opacity-50`.

**Глобальный рендеринг шрифтов:** `globals.css` хранит базу читаемости: `font-size: 16px`, `line-height: 1.5`, `letter-spacing: 0`, `text-rendering: optimizeLegibility`, без принудительного размытия через `antialiased` на рабочих экранах.
...
**Hover (если кликабельная):** обычные карточки могут оставаться на `hover:border-primary/40 transition-colors`.

### 2.1.1 ARAY edge-hover для админки

С 05.05.2026 для рабочих карточек админки согласован единый hover-эффект Армана: тонкая живая рамка строго по краю карточки.

```tsx
<Link className="admin-liquid-surface admin-liquid-interactive rounded-2xl ...">
  ...
</Link>
```
...
- применяется к рабочим surface/card элементам внутри `admin-content-root`;
- эффект включается только явно через `admin-liquid-interactive` или `admin-edge-hover`; базовый `admin-liquid-surface` остается спокойной статичной поверхностью без анимации;
- цвет берется из выбранной палитры: `--primary`, `--atmo-accent`, `--atmo-glow`;
- рамка тонкая (`1px`), без `box-shadow` и без внешнего мутного ореола;
- hover не двигает карточку: `transform: none`, иначе 1px-рамка попадает на полпикселя и углы выглядят криво;
- кнопки, инпуты и формы остаются на стандартных button/form паттернах, этот эффект на них не переносить;
- shell/nav, fixed/absolute/sticky overlay и mobile bottom dock исключены;
- `prefers-reduced-motion` отключает вращение рамки.

### 2.1.2 Mobile bottom nav glass

С 05.05.2026 нижняя навигация админки (`.admin-mobile-dock`) использует стекло из 4AYKA-сниппета: полупрозрачный `bg-card`, `blur(35px) saturate(180%)`, мягкий верхний radial-блик, без постоянной тяжелой тени.

Правила:

- на mobile ARAY открывается только центральной кнопкой bottom nav; отдельный fixed ARAY input dock скрыт до `lg`;
- tap по ARAY = открыть единый assistant (`aray:open`);
- long-press по ARAY = голосовой режим (`aray:voice`);
- нижнее меню не получает ARAY edge-hover: это навигационный shell, а не карточка контента.
...
```tsx
<button className="inline-flex items-center justify-center gap-2 border border-primary/45 bg-primary/10 text-primary px-5 h-11 rounded-xl text-sm font-semibold hover:bg-primary/15 transition-colors">
...
### 2.3.1 Selected controls

Обычные выбранные состояния нельзя делать залитой primary-кнопкой. Для вкладок, фильтров, способов оплаты, карточек выбора, сегментов, настроек и быстрых переключателей используем мягкий активный стиль:
...
Ошибки и предупреждения в админке показываем через reusable классы из `app/globals.css`: `admin-alert`, `admin-alert-danger`, `admin-alert-info`. Текст остаётся `foreground`, цвет получает иконка; не собираем резкие плашки вида `bg-destructive/10 text-destructive`.

```tsx
<div className="admin-alert admin-alert-danger flex items-start gap-3 p-3 text-sm">
  <AlertCircle className="h-4 w-4" />
  <p>Проверьте оплату перед закрытием заказа.</p>
</div>
```
...
Переключатель должен выглядеть одинаково на мобильном, планшете и десктопе. Нельзя делать отдельный mobile-style для switch: если на десктопе track маленький и спокойный, на телефоне он остаётся таким же. Touch target обеспечивается всей строкой настройки (`SettingRow`/карточка), а не раздуванием самого track.

```tsx
<button
  type="button"
  data-state={checked ? "on" : "off"}
  className="aray-toggle aray-toggle-md"
...
- `aray-toggle-md` — стандарт для строк настроек.
- `aray-toggle-lg` — только если весь раздел использует крупные controls.
- Никаких breakpoint-классов на размере switch (`sm:`, `lg:`) без отдельного согласования.
- Глобальные mobile touch-target правила не должны менять размер switch.
- Перед завершением раздела проверить switch минимум на 390px и 1024px: размер, цвет, положение thumb должны совпадать.
...
**Важно:** ARAY edge-hover из 2.1.1 не является возвратом ARAYGLASS. Это только тонкая палитровая рамка по краю интерактивной карточки, без внешних теней, blur-слоев и новых кнопочных эффектов.
...
- \[ \] Admin edge-hover остается edge-only и opt-in: только `admin-liquid-interactive` / `admin-edge-hover` на важных action-карточках, цвет из палитры, без box-shadow и без переноса на кнопки/инпуты/формы/таблицы
...
- \[ \] Toggle switch одинаковый на mobile/desktop; touch target даёт строка, не раздутый track
...
<div className="bg-card border border-border rounded-2xl overflow-hidden">
  ...
  <div className="flex items-center justify-between gap-3 p-4 border-b border-border flex-wrap">
    ...
    <div className="flex items-center gap-2">
      ...
      {/* Local filters only. Global search lives in AppHeader. */}
    </div>
  </div>
...
- Поиск по сущностям (заказы, клиенты, товары, разделы) живёт в хедере админки. Внутри разделов оставляем только фильтры, вкладки, сортировки и bulk-действия.

### 9.1.1 Header search (умный поиск админки)

**Паттерн согласован 02.05.2026.** Поиск — фирменная навигационная фича админки, а не локальный инпут в каждом разделе.

**Desktop (`xl+`):**
- В центре `AppHeader` стоит настоящий input, не кнопка-заглушка.
- Рекомендуемая ширина: `w-full max-w-3xl`. Шире делать только после проверки, что не давит заголовок слева и действия справа.
- Пользователь может сразу писать в строке. Клик и `Ctrl+K` фокусируют input.
- Результаты выпадают прямо под строкой: разделы, заказы, клиенты, товары.
- Визуальный язык: `bg-card`, `border-border`, `rounded-2xl`, строки `rounded-xl`, иконки lucide, без `backdrop-blur`.

**Mobile / tablet (`< xl`):**
- В хедере остаётся компактная иконка поиска.
- По нажатию открывается утверждённый search popup / side panel.
- Inline input в мобильный хедер не ставить: он съедает место, ломает заголовок и действия страницы.

**Правила разделов:**
- Не добавлять локальные поисковые поля в `orders`, `clients`, `products` и другие сущностные разделы, если запрос можно обработать глобальным поиском.
- Внутри разделов допустимы только контекстные фильтры: статус, вкладки, сортировка, тип заказа, bulk-действия.
- Если разделу нужен особый поиск по сложной таблице, сначала описать исключение здесь.

**Smart Search v2 (зафиксировано 06.05.2026):**
- Поиск обязан учитывать текущую страницу. На пустом запросе он показывает не одинаковые карточки везде, а 4-6 релевантных быстрых переходов, фильтров или действий для текущего раздела.
- Результаты сортируются по пользе: точное совпадение номера/телефона/названия, затем сущности текущего раздела, затем соседние сущности, затем разделы и настройки.
- Desktop и mobile используют один контракт результатов, но разные поверхности: dropdown под header input на `xl+`, slide/panel на `< xl`.
- Локальные поисковые поля не плодить. Если разделу нужен свой сложный табличный поиск, он должен быть связан с глобальным поиском или явно описан как исключение.
- Поиск не показывает данные, к которым роль не имеет доступа, и не выдумывает сущности. Пустой результат объясняет, что можно ввести или какой быстрый фильтр нажать.

### 9.1.2 Client CRM list

**Паттерн согласован 02.05.2026.** `/admin/clients` — не просто список пользователей, а спокойная CRM-база покупателей.

- Первый слой: метрики базы, сегменты, затем карточки клиентов. Локального поиска нет — поиск живёт в хедере.
- Сегменты вычисляются из существующих данных: `Все`, `Новые`, `С заказами`, `Повторные`, `VIP`, `Внимание`.
- До отдельного решения не добавлять поля в БД для рейтинга/статуса клиента. Рейтинг и сегмент можно считать из заказов, выручки и заполненности контактов.
- Строка клиента показывает только главное: имя, контакты, сегмент, заказы, выручку, рейтинг. Вторичные действия прятать в меню `...`.
- Раскрытие строки — для истории заказов и контекста. Не делать отдельную витрину иконок на каждой строке.
...
### 12.5 Автоматическая защита

`npm run quality` запускает `scripts/validate-design-system.js`. Проверка смотрит новые строки в изменённых UI-файлах (`app/`, `components/`, `lib/`) и ловит дрейф:
- серые/белые/чёрные utility-цвета вместо токенов;
- hex, `rgb()`, `rgba()` в UI;
- устаревшие радиусы `rounded-sm`, `rounded-md`, `rounded-lg`;
- постоянные `shadow-sm/md/lg`, `backdrop-blur`, `bg-gradient-to-*`;
- deprecated `arayglass-glow` и `arayglass-shimmer`.

Проверка не заменяет дизайн-ревью, но останавливает случайные отклонения до коммита. Если проверка упала — сначала привести код к существующему паттерну. Если паттерна действительно нет — обновить DESIGN_SYSTEM.md и согласовать.
...
A: Только framer-motion (уже в зависимостях), CSS `transition-colors` или согласованный opt-in ARAY edge-hover через `admin-liquid-interactive` / `admin-edge-hover`. Нет blur, нет box-shadow/glow-ореолов, нет pulse на постоянных элементах.
...
> **Последнее обновление:** 04.05.2026 (версия 1.5: закон единого Admin Page Frame для одинаковой резиновой системы страниц).
> **Следующее запланированное обновление:** при утверждении нового визуального паттерна, которого нет в системе.
## ARAY Popup Standard - 2026-05-07

- Все новые popup/drawer/sheet в admin строятся на `AdminModal` или `SidePanel` и визуально наследуют `admin-popup-liquid`.
- Mobile overlays открываются как ARAY bottom sheet: blur backdrop, safe-area отступы, верхний handle, без тесных floating cards поверх dock.
- Терминал, уведомления, меню, настройки и будущие модули не должны плодить разные типы попапов. Новый ручной overlay сначала добавляется в общий primitive, затем используется в разделе.
- PWA icon: ARAY/admin/app контексты используют ARAY icon, публичный ПилоРус сайт сохраняет свой favicon/logo.
