...
function getDirectApiBase(settings?: DirectSettings) {
  return (settingValue(settings, ["yandex_direct_api_url", "direct_api_url"]) || process.env.YANDEX_DIRECT_API_URL || "https://api.direct.yandex.com/json/v5/")
    .trim()
    .replace(/\/?$/, "/");
}

function getDirectClientLogin(settings?: DirectSettings) {
  return settingValue(settings, ["yandex_direct_client_login", "direct_client_login"]);
}

function directErrorMessage(data: DirectErrorPayload | null) {
  return data?.error?.error_detail || data?.error?.message || "Yandex Direct API не ответил";
}
...
export async function callYandexDirect<T>(
  ...
  if (!token) throw new Error("Yandex Direct OAuth token is missing for current tenant");
  ...
  const res = await fetch(`${getDirectApiBase(options.settings)}${service.replace(/^\/+/, "")}`, {
    method: "POST",
    cache: "no-store",
    headers,
    body: JSON.stringify(payload),
  });
  const data = (await res.json().catch(() => null)) as (T & DirectErrorPayload) | null;

  if (!res.ok || data?.error) {
    throw new Error(directErrorMessage(data));
  }
  return data as T;
}
...
export async function getYandexDirectStatus(settings?: DirectSettings): Promise<YandexDirectStatus> {
  ...
  try {
    const data = await callYandexDirect<{ result?: { Campaigns?: DirectCampaign[] } }>("campaigns", body, { settings });
