...
export function ArayWidget({ page, productName, cartTotal, enabled = true, staffName, userRole, adminNavigation }: ArayWidgetProps) {
  ...
  const sendMessage = useCallback(async (text?: string, options?: SendMessageOptions) => {
    ...
    const visibleMsg = (options?.displayText || msg).trim();
    const hiddenContext = options?.context?.trim();
    ...
    if (options?.openUrl) openArayTarget(options.openUrl);
    ...
    try {
      ...
      const finalActions = mergeArayActions(options?.actions, actions);
