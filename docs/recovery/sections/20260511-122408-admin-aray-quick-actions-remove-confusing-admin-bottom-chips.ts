...
const PAGE_QUICK_ACTIONS: Record<string, QuickTemplate[]> = {
  ...
  "/admin/promotion": [
    { label: "Метрика", prompt: "метрика" },
    { label: "Цели", prompt: "цели метрики" },
    { label: "Direct", prompt: "direct" },
    { label: "SEO", prompt: "seo" },
    { label: "Организация", prompt: "организация" },
    { label: "Спрос", prompt: "спрос" },
  ],
  "/admin/finance": [
...
export function buildAdminArayNavigation({
  ...
  const promptLimit = matchedPromptActions.length > ARAY_PROMPT_ACTION_LIMIT
    ? ARAY_QUICK_ACTION_LIMIT
    : ARAY_PROMPT_ACTION_LIMIT;
  const promptActions = matchedPromptActions
    .slice(0, promptLimit)
