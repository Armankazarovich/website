...
import { buildYandexDirectDraft, normalizeDirectDraftOptions, type DirectDraftOptions } from "@/lib/direct-campaign-draft";
...
async function readExportOptions(req: Request) {
  const body = await req.json().catch(() => ({}));
  return normalizeDirectDraftOptions((body?.options || body) as Record<string, unknown>);
}
...
export async function POST(req: Request) {
  ...
  try {
    const options = await readExportOptions(req);
    ...
    const result = await exportYandexDirectDraft({ draft, settings, options });
