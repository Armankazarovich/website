  ...
  return (
    <form onSubmit={handleLogin} className="space-y-3">
      ...
      <button
        type="submit"
        disabled={loading}
        className="bg-primary text-primary-foreground px-4 py-3 sm:py-2 rounded-xl text-sm font-medium hover:bg-primary/90 transition-colors disabled:opacity-50"
      >
...
export default function CheckoutPage() {
  ...
  const [pushState, setPushState] = useState<
    "idle" | "busy" | "done" | "denied" | "unsupported"
  >("idle");
    ...
    try {
      ...
      const sub =
        ...
        (await reg.pushManager.subscribe({
          ...
          applicationServerKey: b64(vapidKey) as unknown as BufferSource,
        }));
      ...
      if (k && a) {
        ...
        await fetch("/api/push/subscribe", {
          ...
          body: JSON.stringify({
            ...
            keys: { p256dh: toB64(k), auth: toB64(a) },
          }),
        });
      }
      setPushState("done");
    ...
    fetch("/api/site-settings")
      ...
      .then((data) => {
        ...
        const nextSettings: CheckoutSettings = {
          allowPickup: data?.checkout_allow_pickup !== "false",
          allowDelivery: data?.checkout_allow_delivery !== "false",
          showPromo: data?.checkout_show_promo !== "false",
          allowGuest: data?.checkout_allow_guest !== "false",
          requireComment: data?.checkout_require_comment === "true",
        };
        if (!nextSettings.allowPickup && !nextSettings.allowDelivery) {
          nextSettings.allowDelivery = true;
        }
        setCheckoutSettings(nextSettings);
        if (!nextSettings.allowGuest) setAuthMode("login");
        if (!nextSettings.allowDelivery && nextSettings.allowPickup) {
          setDeliveryType("pickup");
          setValue("address", `Самовывоз: ${data?.address || pickupAddress}`);
        }
        if (!nextSettings.allowPickup && nextSettings.allowDelivery) {
  ...
  useEffect(() => {
    if (!checkoutSettings.allowGuest && authMode === "guest") {
      setAuthMode("login");
    }
  }, [authMode, checkoutSettings.allowGuest]);

  useEffect(() => {
    if (
      !checkoutSettings.allowDelivery &&
      checkoutSettings.allowPickup &&
      deliveryType !== "pickup"
    ) {
      setDeliveryType("pickup");
      setValue("address", `Самовывоз: ${pickupAddress}`);
    }
    if (
      !checkoutSettings.allowPickup &&
      checkoutSettings.allowDelivery &&
  ...
  const onSubmit = async (data: CheckoutForm) => {
    if (!session?.user && !checkoutSettings.allowGuest) {
      toast({
        title: "Войдите в кабинет",
        description:
          "Для оформления заказа без гостевого режима нужен вход или регистрация.",
        variant: "destructive",
      });
      return;
    }
    if (checkoutSettings.requireComment && !data.comment?.trim()) {
      toast({
        title: "Добавьте комментарий",
        description: "Комментарий обязателен для текущего режима оформления.",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      if (checkoutSettings.showPromo && data.promoCode?.trim()) {
        const promoLine = `Промокод: ${data.promoCode.trim()}`;
        comment = comment ? `${promoLine}\n${comment}` : promoLine;
      }
      ...
      <div className="container py-16 max-w-lg mx-auto">
        ...
        <div className="bg-card border-2 border-primary/20 rounded-2xl p-6 text-center mb-4">
          ...
          <p className="text-xs text-muted-foreground">
            Сохраните его для отслеживания
          </p>
        </div>
        ...
        {pushState !== "unsupported" && pushState !== "done" && (
          <div
            className={`rounded-2xl p-4 mb-4 border ${pushState === "denied" ? "bg-muted/30 border-border" : "bg-primary/5 border-primary/20"}`}
          >
            {pushState === "denied" ? (
              <p className="text-xs text-muted-foreground text-center inline-flex items-center justify-center gap-2">
                <Info className="w-4 h-4 shrink-0" />
                <span>
                  Разрешите уведомления в настройках браузера — и мы пришлём
                  статус заказа
                </span>
              </p>
            ) : (
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-muted-foreground mt-0.5">
                    Пришлём уведомление когда заказ будет готов
                  </p>
                </div>
                <button
        ...
        <div className="bg-muted/50 rounded-2xl p-4 mb-6 text-sm text-muted-foreground space-y-2">
          ...
          <div className="flex items-start gap-2">
            ...
            <span>
              По ссылке выше — введите <strong>#{orderNum}</strong> и номер
              телефона
            </span>
          </div>
          <div className="flex items-start gap-2">
  ...
  const deliveryOptionCount =
    Number(checkoutSettings.allowDelivery) +
    ...
    <div className="container py-8 max-w-4xl">
      ...
      {!session?.user && (
        <div className="bg-card rounded-2xl border border-border p-5 mb-6">
          ...
          {authMode === "register" && (
            <CheckoutRegisterForm onSuccess={() => {}} />
          )}
          {authMode === "guest" && (
            <p className="text-sm text-muted-foreground">
              Оформляете как гость. Статус заказа можно отследить по номеру
              заказа и телефону на странице{" "}
      ...
      {session?.user && (
        <div className="bg-emerald-50 dark:bg-emerald-950/30 border border-emerald-200 dark:border-emerald-800 rounded-2xl p-4 mb-6 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              Заказ сохранится в личном кабинете
            </p>
          </div>
        </div>
      )}
        ...
        >
          ...
          <div className="bg-card rounded-2xl border border-border p-5">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              >
                <User
                  className={`w-6 h-6 ${clientType === "individual" ? "text-primary" : ""}`}
                />
              ...
              >
                <Building2
                  className={`w-6 h-6 ${clientType === "company" ? "text-primary" : ""}`}
                />
            ...
            <div className="bg-card rounded-2xl border border-primary/30 p-5 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                      ...
                      {...register("inn", {
                        onBlur: async (e) => {
                          ...
                          try {
                            ...
                            if (data.address && deliveryType === "delivery")
                  ...
                  {innError && (
                    <p className="text-xs text-destructive mt-1">{innError}</p>
                  )}
                  {!innError && (
                    <p className="text-xs text-muted-foreground mt-1">
                      10 цифр для ООО, 12 для ИП — данные заполнятся
                      автоматически
                    </p>
                  )}
                </div>
          ...
          <div className="bg-card rounded-2xl border border-border p-5 space-y-4">
            ...
            <div
              className={`grid ${deliveryOptionCount > 1 ? "grid-cols-2" : "grid-cols-1"} gap-3`}
            >
              {checkoutSettings.allowDelivery && (
                <button
                  type="button"
              ...
              <div className="rounded-xl bg-primary/5 border border-primary/20 overflow-hidden">
                <div className="flex items-start gap-3 p-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-muted-foreground mt-0.5">
                      {workingHours} · Есть погрузчик · Предзвоните перед
                      приездом
                    </p>
                  </div>
                </div>
            ...
            <div className="space-y-3">
              ...
              <label className="flex items-start gap-3 cursor-pointer p-3 rounded-xl border border-border hover:border-primary/50 transition-colors has-[:checked]:border-primary has-[:checked]:bg-primary/5">
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">
                    Для ИП и ООО. Выставим счёт с реквизитами и НДС.
                  </p>
                </div>
              </label>
            </div>
          ...
          <div className="bg-card rounded-2xl border border-border p-5 space-y-4">
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-0.5">
                Менеджер свяжется удобным для вас способом
              </p>
            </div>
                ...
                [
                  ...
                  },
                  {
                    ...
                    icon: (
                      <svg className="w-5 h-5" viewBox="0 0 24 24" fill="none">
                        <path
                          d="M20.5 3.5A11.9 11.9 0 0012 0C5.4 0 0 5.4 0 12c0 2.1.6 4.2 1.6 6L0 24l6.2-1.6C8 23.4 10 24 12 24c6.6 0 12-5.4 12-12 0-3.2-1.2-6.2-3.5-8.5zm-8.5 18.4c-1.8 0-3.6-.5-5.1-1.4l-.4-.2-3.7 1 1-3.6-.2-.4A9.9 9.9 0 012.1 12C2.1 6.5 6.5 2 12 2s9.9 4.5 9.9 10-4.4 9.9-9.9 9.9zm5.4-7.4c-.3-.1-1.7-.9-2-.9-.2-.1-.4-.1-.6.1-.2.2-.7.9-.9 1.1-.1.2-.3.2-.6.1-.3-.1-1.2-.5-2.3-1.4-.9-.8-1.4-1.7-1.6-2-.2-.3 0-.5.1-.6l.4-.5c.1-.2.2-.3.2-.5 0-.2-.5-1.3-.7-1.8-.2-.4-.4-.4-.6-.4h-.5c-.2 0-.5.1-.7.3-.3.3-1 1-1 2.4s1 2.7 1.2 2.9c.1.2 2.1 3.2 5.1 4.5.7.3 1.3.5 1.7.6.7.2 1.4.2 1.9.1.6-.1 1.7-.7 2-1.4.2-.6.2-1.2.1-1.4-.1 0-.3-.1-.6-.2z"
                          fill="currentColor"
                          opacity=".9"
                        />
                      </svg>
          ...
          {checkoutSettings.showPromo && (
            <div className="bg-card rounded-2xl border border-border p-5">
              <Label htmlFor="promoCode">Промокод</Label>
              <Input
                id="promoCode"
                placeholder="Если менеджер выдал код"
                className="mt-1 uppercase"
                {...register("promoCode")}
              />
              <p className="text-xs text-muted-foreground mt-1">
                Менеджер проверит промокод перед подтверждением заказа.
              </p>
            </div>
          )}
          ...
          <div className="bg-card rounded-2xl border border-border p-5">
            <Label htmlFor="comment">
              ...
              {checkoutSettings.requireComment && (
