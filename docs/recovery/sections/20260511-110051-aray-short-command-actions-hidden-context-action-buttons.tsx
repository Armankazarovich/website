...
export function ArayWidget({ page, productName, cartTotal, enabled = true, staffName, userRole, adminNavigation }: ArayWidgetProps) {
  ...
  useEffect(() => {
    ...
    setShowMessages(true);
    window.setTimeout(() => sendMessageRef.current?.(text), 120);
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [startChat]);
  ...
  const sendMessage = useCallback(async (text?: string) => {
    ...
    if ((!msg && messageAttachments.length === 0) || loading || attachmentsBusy) return;
    setInput("");
    setAttachments([]);
    const instantAdminTarget = isAdmin && messageAttachments.length === 0
      ? findInstantAdminNavigationTarget(msg, adminNavigation)
      : null;
  ...
  const handleChipClick = useCallback((label: string) => {
    ...
    sendMessage(action?.prompt || label);
  }, [adminQuickActions, handleOpenAdminPage, sendMessage]);
          ...
          <>
            ...
            <motion.div
                  ...
                  <div className="flex flex-col items-center justify-center gap-4 py-6 px-4 animate-in fade-in zoom-in-95 duration-300">
                    ...
                    {chips.length > 0 && messages.length <= 1 && (
                      <div className="flex max-w-full gap-2 flex-wrap justify-center mt-1">
                        {chips.slice(0, ARAY_MAX_SMART_CHIPS).map(q => (
                    ...
                    {isAdmin && adminNavigation && messages.length <= 1 && (
              ...
              <div className="px-4 py-3 shrink-0" style={{ borderTop: `1px solid ${dividerColor}` }}>
                ...
                {showMessages && isAdmin && adminNavigation && !loading && (
                  <div className="mb-2" data-no-page-swipe>
                    <ArayAdminNavigationStrip
                      navigation={adminNavigation}
                      onOpenPage={handleOpenAdminPage}
                      isDark={isDark}
