...
import { Send, Loader2, RotateCcw, Mic, MicOff, ShoppingCart, ExternalLink, LayoutGrid, Package, MapPin, Phone, Volume2, VolumeX, MessageSquare, ChevronDown, ChevronLeft, ChevronRight, ShieldCheck, CheckCircle2, XCircle, Paperclip, FileText, Image as ImageIcon, Trash2, FileAudio, Film, FileArchive } from "lucide-react";
...
const ARAY_MAX_SMART_CHIPS = 3;
const ARAY_MAX_NAV_LINKS = 2;
...
function buildPromotionPromptPayload(command: string): ArayPromptPayload | null {
  ...
  if (normalized.includes("метрик") || normalized.includes("цели")) {
    ...
    return {
      ...
      localReply: goals
        ? "Давай спокойно: цели открываем через счетчик, иначе Яндекс часто дает 403. Я открыл список счетчиков. Выбери счетчик сайта, а если OAuth Метрики подключен, ARAY сам создаст нужные цели и сохранит ID."
        : "Давай спокойно: сначала найдем счетчик Метрики для сайта. Я открыл список счетчиков. Если OAuth подключен, ARAY сам подтянет счетчик, создаст цели и сохранит готовность.",
      context: [
      ...
      actions: [
        { type: "navigate", url: "https://metrika.yandex.ru/list", label: "Открыть Метрику", icon: "external" },
        { type: "navigate", url: "https://metrika.yandex.ru/list", label: "Найти счетчик", icon: "target" },
  ...
  if (normalized.includes("direct") || normalized.includes("директ")) {
    return {
      ...
      actions: [
        { type: "navigate", url: "/api/admin/direct/draft?grouping=category&maxGroups=3&maxAdsPerGroup=1&maxKeywordsPerGroup=5", label: "Мастер Direct", icon: "target" },
        { type: "navigate", url: "https://direct.yandex.ru/dna/grid/campaigns", label: "Открыть Direct", icon: "external" },
      ],
    };
  }
  if (normalized.includes("организа")) {
    return {
      ...
      localReply: "Начнем с организации: открой Яндекс Бизнес и проверь, есть ли профиль этой компании. Если найдем правильный профиль, ARAY сможет сохранить ID и использовать контакты в рекламе.",
      context: [
  ...
  if (normalized.includes("спрос") || normalized.includes("wordstat") || normalized.includes("вордстат")) {
    return {
      text: "спрос",
      displayText: "спрос",
      localReply: "Открыл маршрут по спросу. Сначала смотрим Wordstat: частотность, регионы и сезонность. Если подключим API, ARAY будет подтягивать это в аналитику сам.",
      context: [
...
function mergeArayActions(...groups: Array<ArayAction[] | undefined>): ArayAction[] {
  ...
  for (const group of groups) {
    for (const action of group ?? []) {
      ...
      const key = `${safeAction.type}:${safeAction.url || ""}:${safeAction.label}`;
...
function getAttachmentActionHint(file: ArayAttachment) {
  ...
  if (file.kind === "audio") return "Аудио принято. Расшифровка и анализ записи идут через голосовой контур ARAY.";
  if (file.kind === "video") return "Видео принято. ARAY сможет использовать его для разборов, рекламы и сценариев.";
  if (file.kind === "archive") return "Архив принят. Авто-распаковка будет подключена отдельным безопасным обработчиком.";
  if (file.kind === "text") return "Текст прочитан и передан ARAY.";
  ...
  return "Файл принят. Если нужно, ARAY подскажет следующий безопасный способ обработки.";
}
...
const INSTANT_ADMIN_NAV_RULES: Array<{ href: string; keywords: string[] }> = [
  { href: "/admin/orders/new?mode=market", keywords: ["бирж", "маркет", "рынок пиломатериал", "пиломатериалов", "внешняя биржа"] },
...
function findInstantAdminNavigationTarget(
  ...
  const conciseNavIntent = text.split(" ").length <= 4 && INSTANT_ADMIN_NAV_RULES.some((rule) =>
    rule.keywords.some((keyword) => text.includes(normalizeArayNavText(keyword)))
  );
  if (!hasIntent && !conciseNavIntent) return null;
...
function ArayAdminNavigationStrip({
  ...
  if (!navigation || navigation.nearbyPages.length === 0) return null;

  const border = isDark ? "rgba(255,255,255,0.10)" : "rgba(0,0,0,0.08)";
  const surface = isDark ? "rgba(255,255,255,0.045)" : "rgba(0,0,0,0.035)";
  const text = isDark ? "rgba(255,255,255,0.78)" : "rgba(15,15,15,0.78)";
  const muted = isDark ? "rgba(255,255,255,0.48)" : "rgba(15,15,15,0.48)";
  const visiblePages = navigation.nearbyPages.filter((page) => !page.active).slice(0, ARAY_MAX_NAV_LINKS);
  if (visiblePages.length === 0) return null;

  return (
    <div className="w-full max-w-full space-y-2">
      <div className="flex flex-col items-center gap-0.5 px-1 text-center">
        <span className="max-w-full truncate text-[11px] font-semibold uppercase tracking-[0.08em]" style={{ color: muted }}>
          Быстрый переход
        </span>
      </div>
      <div className="grid w-full max-w-full grid-cols-1 gap-1.5 min-[380px]:grid-cols-2">
        {visiblePages.map((page) => (
          <button
            key={page.href}
            type="button"
            onClick={() => onOpenPage(page.href)}
            className="aray-quick-chip inline-flex min-h-10 max-w-full items-center justify-center gap-1.5 rounded-2xl px-3 py-2 text-[12px] font-medium leading-tight transition-colors active:scale-95"
            style={{
              color: text,
              background: surface,
    ...
    return new Promise(async (resolve) => {
      ...
      const resetSilenceTimer = () => {
        ...
        if (fullText || interimText) {
          silenceTimer.current = setTimeout(finishWithText, 1200);
        }
      };

      r.onstart = () => { setActive(true); };
      let restartCount = 0;
      const maxRestartsFast = 2;
...
export function ArayWidget({ page, productName, cartTotal, enabled = true, staffName, userRole, adminNavigation }: ArayWidgetProps) {
  ...
  const visibleChipLimit = ARAY_MAX_SMART_CHIPS;
  const showSmartChips = !isAdmin && chips.length > 0;
  ...
  const openArayTarget = useCallback((url: string): "internal" | "external" | null => {
    const target = sanitizeArayUrl(url);
    if (!target) return null;

    const internalPath = toInternalAppPath(target);
    if (internalPath) {
      const isWorkspacePath = internalPath.startsWith("/admin") || internalPath.startsWith("/cabinet");
      if (isWorkspacePath) {
  ...
  const applyArayOpenCommands = useCallback((rawText: string, actions: ArayAction[]) => {
    ...
    return {
      openedInternal,
      openedExternal,
      openedAny: openedInternal || openedExternal,
    };
  }, [openArayTarget]);
  ...
  const buildFinalArayText = useCallback((
    parsedText: string,
    isError: boolean,
    commandState: { openedInternal: boolean; openedExternal: boolean; openedAny: boolean }
  ) => {
    ...
    if (commandState.openedExternal) return clean || "Показал рядом. Если Яндекс открылся вкладкой, продолжай там.";
    return clean || "Готово.";
  }, []);
  ...
  const sendMessage = useCallback(async (text?: string, options?: SendMessageOptions) => {
    ...
    setInput("");
    setAttachments([]);
    const instantAdminTarget = isAdmin && messageAttachments.length === 0
    ...
    if (localReply && messageAttachments.length === 0) {
      ...
      const finalActions = mergeArayActions(effectiveOptions?.actions);
      const assistantMsg: Message = {
        id: (Date.now() + 1).toString(),
        role: "assistant",
        content: localReply,
      ...
      if (voiceModeRef.current === "voice") speak(localReply);
      if (isMobile) stopAraySpeech();
      return;
    }
    ...
    try {
      ...
      const finalActions = mergeArayActions(effectiveOptions?.actions, actions);
  ...
  }, [adminNavigation, adminNavigationPayload, applyArayOpenCommands, attachments, attachmentsBusy, buildFinalArayText, cartTotal, input, isAdmin, isMobile, loading, messages, micListen, open, openArayTarget, pathname, productName, saveMessageToDB, speak]);
  ...
  const handleConfirmAction = useCallback(async (confirmation: ArayConfirmationDraft) => {
    ...
    try {
      ...
      const { text: parsedText, actions } = parseMessageActions(textWithoutConfirmations);
      const commandState = applyArayOpenCommands(rawText, actions);
      const finalActions = mergeArayActions(actions);
      ...
      if (voiceModeRef.current === "voice" && finalParsed && !isError) {
        speak(finalParsed);
      }
    } catch {
  ...
  }, [adminNavigationPayload, applyArayOpenCommands, buildFinalArayText, cartTotal, loading, messages, pathname, productName, saveMessageToDB, speak]);
              ...
              <div className="flex-1 flex flex-col overflow-hidden">
                ...
                {!showMessages && (
                  <div className="flex flex-col items-center justify-center gap-4 py-6 px-4 animate-in fade-in zoom-in-95 duration-300">
                    ...
                    {/* Быстрые чипы */}
                    {showSmartChips && messages.length <= 1 && (
