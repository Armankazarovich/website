...
## Finance + Analytics Sync - 2026-05-11

Status: `READY_LOCAL / NEEDS_VISUAL_APPROVAL`

- [x] `/api/admin/analytics` made tenant-aware: orders, products, statuses, repeat clients, payment/contact stats and attribution are scoped by current tenant.
- [x] Analytics now reuses the same honest Direct/Metrika source layer as finance: Direct spend, clicks, CTR, CPC, conversions, CPA, Direct ROAS, Metrika visits, goals, conversion, regions and sources.
- [x] `/admin/analytics` now shows a compact `Реклама и аналитика` block so owner sees orders, UTM revenue, ad spend and Metrika traffic in one place.
- [x] Empty states stay honest: if Direct/Metrika OAuth or counter is missing, UI says what is missing and does not invent spend, demand, conversion or regions.
- [ ] Browser visual approval still needed for desktop/mobile before calling this analytics slice accepted.

## ARAY Connector Bundles - 2026-05-11

Status: `READY_LOCAL / NEEDS_VISUAL_APPROVAL`

- [x] `lib/aray-provider-matrix.ts` now groups providers into one-click business bundles: Yandex growth, Google growth, inbox/reputation, finance/accounting and ARAY core.
- [x] `/api/admin/aray/connectors` returns `bundles` together with provider status so future onboarding, chat actions and dashboards can reuse one source of truth.
- [x] `/admin/aray/connectors` shows a simple bundle section before the technical provider matrix: what ARAY can automate, what the owner must confirm and where to continue.
- [x] Safety rule kept: login OAuth and service OAuth stay separate; money, public replies, legal sends and ad launch still require explicit confirmation.
- [x] Added `/api/admin/aray/connectors/yandex`: one honest Yandex overview for Direct, Metrika, counter, goals, public domain and organization.
- [x] Added `YandexGrowthConnectorPanel`: Direct OAuth, Metrika OAuth, refresh, save detected counter and create missing Metrika goals from one place.
- [x] One-click automation is intentionally limited to safe work: save IDs and create Metrika goals after OAuth. Direct launch, public organization changes and money actions remain confirmation-only.
- [ ] Next slice: connect organization discovery/confirmation through the official Yandex Business access path, then reuse the same status in Promotion and Analytics.
