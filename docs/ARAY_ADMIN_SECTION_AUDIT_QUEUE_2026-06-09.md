...
- `Fixed` PiloRus price-size conflict and catalog cache pass on 2026-06-19: reran the catalog audit against the 2026-06-19 Pilmos/Pil-mos snapshot. Current local public catalog is `69` active cards and `745` variants with `0` missing products, `0` extra products, `0` missing variants, `0` extra variants, and `0` bad size labels. Found `22` legacy timber variants where both `m3` and `piece` prices existed but the piece price differed from `pricePerCube / piecesPerCube` by more than 25%; the repeatable Pilmos snapshot builder and deploy data migration now clear only those conflicting piece prices for timber variants, leaving true piece-priced sheet/finish products intact. After migration the conflict audit is `0`. Admin product readiness no longer requires a separate piece price for cube-priced timber cards. Public catalog/product middleware now preserves tenant headers for admin/API/preview only, root layout uses the explicit default PiloRus tenant, catalog/product pages get `Cache-Control: public, s-maxage=60, stale-while-revalidate=300` except catalog text search, and the catalog page no longer prefetches category/type/pagination URLs in the background. Local production HTTP check: `/catalog` warmed at about `0.67s`, `/catalog?category=sosna-el` at about `0.67s`, product page at about `0.50s`, `/catalog?search=...` remains `no-store`, and `/api/yml` stays `public, max-age=3600` with `744` offers and no bad absolute URLs. Browser check passed for local `/catalog`: 20 cards, cart buttons present, `m3` text present, `0` horizontal overflow, and no console errors. Verified with `npx tsc --noEmit`, `npm run nav:check`, `npm run text:check`, `npm run build`, and `npm run quality:full`.
- `Verified` PiloRus production deploy on 2026-06-19 after commit `3216903`: `npm run deploy` pushed the price/cache fix to `main`. Live smoke passed for `13` routes at `https://pilo-rus.ru`, including home, catalog, cart, checkout, stories, services, PWA manifest/icons, stories API, admin auth gates, and calculator-to-cart API. Live HTTP checks confirmed `/catalog`, `/catalog?category=sosna-el`, and `/product/doska-obreznaya-iz-sosny-i-eli` return `200` with `Cache-Control: public, s-maxage=60, stale-while-revalidate=300`, `/catalog?search=...` remains `no-store`, admin `/admin/products`, `/admin/import`, `/admin/orders`, `/admin/marketing`, and `/admin/analytics` redirect unauthenticated users to `/login`, `/sitemap.xml` and `/robots.txt` return `200`, and `/404` returns `404`. Production YML returns `744` offers with `0` bad absolute URLs. Browser production checks passed with `npm run browser:mobile:check:prod` and `npm run browser:cart:check:prod`.
- `Fixed` PiloRus timber grade labels on 2026-06-21: checked the manager-reported variant selector issue for `Брус обрезной из сосны и ели` against the live Pilmos YML source `https://pilmos.ru/wp-content/uploads/feed001.xml` dated `2026-06-20T13:34`. The old snapshot mixed dry `1 сорт` m3 variants with piece-priced rows and mislabeled two `9000` Pilmos rows as `сорт 1`; the feed identifies those rows (`2771`, `5529`) as `3 сорт`. The canonical snapshot now publishes this card with explicit labels on every active variant: `сорт 1`, `Архангельский лес, сорт 1`, and `сорт 3`; dry `1 сорт` sizes now use Pilmos `20000` m3 source price -> PiloRus `19800` ₽/m3 with matching piece prices, while `8910` ₽/m3 remains only on `сорт 3`. Local migration reapplied cleanly and replaced the old ambiguous active variants. Fresh DB audit for the product: `14` active variants, `0` active variants without `сорт`, `0` cheap `8910` variants outside `сорт 3`, `0` duplicate size labels. HTTP smoke: local product page `200`, catalog category redirect followed to `200`, `/api/catalog/sizes` `200`. Browser plugin DOM smoke could not attach to the in-app webview during this pass, so the verified surface is DB + HTTP + build output. Verified with `npx tsc --noEmit --pretty false`, `npm run quality`, `npm run build`, local data audit, and local HTTP smoke.
- `Fixed` PiloRus variant choice clarity on 2026-06-21: changed public product variant selection from one long mixed `size + sort + length` list into separate buyer controls derived from the existing variant label: `Сорт`, `Сечение`, and `Длина`. This keeps Pilmos-style attribute logic visible without changing catalog data, prices, SEO names, or import rules. Product pages now pick the matching variant when a manager/customer chooses a sort or section; `Цены и размеры` uses the same filters; catalog card size pickers show clean size as the main label and sort as a separate small line. Checked locally on `Доска обрезная из лиственницы` (`63` variants) and Sosna/El/listvennitsa catalog URLs: product page returned `200` and exposed `Сорт`, `Сечение`, `Длина`; category URLs returned `200`. Browser DOM smoke initially verified sort `BC` + section `28×90` selects `28×90×6000 мм, сорт BC`; a later in-app browser tab attach timed out, so final smoke used local HTTP plus the successful earlier DOM check. Verified with `npx tsc --noEmit --pretty false`, `npm run quality`, `npm run build`, `git diff --check`, and local HTTP smoke.
- `Fixed` PiloRus admin stories MOV upload on 2026-06-22: fixed manager-reported MOV upload failures in `/admin/stories` and the shared media library. Story media inputs and media picker uploads now accept explicit `.mov`/`.m4v` extensions, local file kind detection uses extension fallback when Windows/iPhone does not provide a useful MIME type, uploaded MOV/M4V files stay classified as video in stories, and repeated same-file upload retries are possible without reopening the modal. The admin upload endpoint now normalizes `video/quicktime`, `video/x-quicktime`, `video/mov`, empty/octet-stream MOV by extension, and M4V; it still verifies video magic bytes before saving and rejects image/video extension mismatches. Admin video upload limit is raised from `80MB` to `200MB`, and media listing/serving recognizes `.mov`, `.m4v`, `.mp4`, and `.webm` as video. Verified with `npm run content:check`, `npm run stories:check`, `npm run text:check`, `npx tsc --noEmit --pretty false`, and `npm run build`.
- `Fixed` PiloRus stories/CRM mobile manager pass on 2026-06-22: stories video uploads now use chunked admin media upload for files over 8MB, so 20MB+ iPhone MOV files no longer hit the likely `client_max_body_size 20M` proxy ceiling before reaching Next.js. Story-specific video limit is raised to `500MB`, and the same chunked upload path is used from `/admin/stories` and the media library. Public story playback now has a reversible sound toggle, overlay click-to-close, cleaner story message drawer text, shorter staff push previews, and no duplicated generated context in visitor-visible story comments. Mobile `/admin/crm` now opens leads with a usable manager flow: compact stats, `Все` lead lane, visible lead cards, direct `Звонок` / `WhatsApp` / `Почта` actions, stage badge, and per-card mobile stage selector instead of relying on drag-and-drop. Local browser check passed at `390x844`: `/admin/crm` login, leads tab, all-leads filter, card quick actions, and two visible `Этап лида` selectors. Verified with `npm run stories:check`, `npm run content:check`, `npm run text:check`, `npx tsc --noEmit --pretty false`, `git diff --check`, and `npm run build`.

- `Fixed` PiloRus admin first-load speed pass on 2026-06-22: removed the automatic first-load mount of the global ARAY assistant from the admin shell, so the large store assistant/widget code is loaded only when a manager explicitly opens ARAY. The side search panel and smart-search logic now load only after pressing search/Ctrl+K. Admin route prefetch, avatar profile fetch, notification feed, and weather fetch were delayed so they do not compete with the first useful admin screen. `/admin/crm` now opens on leads by default and the orders kanban refresh runs every 60s only while the tab is visible. The admin layout server query now reuses the fresh user lookup for the staff name instead of doing a second DB read. Fresh production build shows `app/admin/layout` at about `145KB` and `app/admin/crm` at about `76KB`, with no direct `aray-widget` or `admin-search-panel` reference in the admin layout chunk. Local authenticated HTTP smoke passed for `/admin/crm` (`287ms`), `/admin/products` (`212ms`), `/admin/import` (`222ms`), `/admin/orders` (`210ms`), `/admin/promotion` (`308ms`), `/admin/analytics` (`201ms`), and `/admin/stories` (`191ms`). Unauthenticated smoke still redirects admin routes to `/login`; public `/` and `/catalog` return `200`. Verified with `npx tsc --noEmit --pretty false`, `npm run design:check`, `npm run nav:check`, `npm run text:check`, `npm run stories:check`, `npm run content:check`, `git diff --check`, and `npm run build`.
